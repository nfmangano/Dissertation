...
% even for non-included files.
 %DIF > 
\makeatletter %DIF > 
\def\@cline#1-#2\@nil{% %DIF > 
  \omit %DIF > 
  \@multicnt#1% %DIF > 
  \advance\@multispan\m@ne %DIF > 
  \ifnum\@multicnt=\@ne\@firstofone{&\omit}\fi %DIF > 
  \@multicnt#2% %DIF > 
  \advance\@multicnt-#1% %DIF > 
  \advance\@multispan\@ne %DIF > 
  \leaders\hrule\@height\arrayrulewidth\hfill %DIF > 
  \cr %DIF > 
  \noalign{\nobreak\vskip-\arrayrulewidth}} %DIF > 
\makeatother %DIF > 
...
\DIFdelbegin \DIFdel{Rather}\DIFdelend \DIFaddbegin \DIFadd{Therefore}\DIFaddend , in this chapter, I \DIFdelbegin \DIFdel{perform }\DIFdelend \DIFaddbegin \DIFadd{report on }\DIFaddend a longer-term qualitative study of Calico in use at software companies \DIFdelbegin \DIFdel{in order }\DIFdelend to not only evaluate its support for the fourteen design behaviors, but also \DIFdelbegin \DIFdel{to }\DIFdelend understand its role within the context of a real-world design environment. A real-world setting \DIFdelbegin \DIFdel{would provide }\DIFdelend \DIFaddbegin \DIFadd{provides }\DIFaddend a group of individuals an opportunity to use Calico together, and \DIFdelbegin \DIFdel{encourage the use of }\DIFdelend \DIFaddbegin \DIFadd{may lead to }\DIFaddend design behaviors that may not appear within the controlled setting. In order to conduct this evaluation, I performed a multi-week qualitative study at three sites, including a commercial open source software company, an interaction design company, and a distributed research group. In this chapter, I report on my findings of how Calico is used over the long term at these three settings, how users incorporated Calico into their own work, and how it affected the way they design.

\DIFaddbegin \DIFadd{After reporting on each site, I report on the design behaviors as they occurred collectively across all three sites. My focus is to examine if the design behaviors occurred  while using Calico, and if so, what features supported those behaviors beyond regular whiteboard use.
}

\DIFaddend The rest of this chapter is organized as follows: Section \ref{chapter:evaluation:overview} describes the setup of the study, the participants, and the data collections used. Sections \ref{chapter:evaluation:deployment1} - \ref{chapter:evaluation:deployment3} presents the results of the field studies framed within the research questions proposed put forward in Section \ref{chapter:evaluation:overview} and Chapter \ref{chapter:research-question}. More specifically, Section \ref{chapter:evaluation:deployment1} describes the findings at the commercial open source software company. Section \ref{chapter:evaluation:deployment2} describes my findings at the interaction design software company. Section \ref{chapter:evaluation:deployment3} describes my findings with the distributed research group. Section \DIFaddbegin \DIFadd{\ref{chapter:evaluation:design-behaviors} collectively reviews the three field sites for evidence of design behaviors and how they were supported by Calico's features. Section }\DIFaddend \ref{chapter:evaluation:summary} summarizes the lessons from this chapter.
...
\begin{enumerate}
	\item are minimally invasive\DIFdelbegin \DIFdel{,
	}\DIFdelend \DIFaddbegin \DIFadd{;
	}\DIFaddend \item are coherent\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend and
...
\DIFdelbegin \DIFdel{Further, I aim to understand how and }\DIFdelend \DIFaddbegin 

\DIFadd{Additionally, this chapter reports on my efforts to further understand }\DIFaddend what design activities Calico \DIFdelbegin \DIFdel{may be }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend used with in the real world. Previous evaluations of Calico involved asking participants to use Calico from start to finish in their designs\DIFdelbegin \DIFdel{, however}\DIFdelend \DIFaddbegin \DIFadd{. However}\DIFaddend , in a longer-term deployment, participants may turn to Calico to sketch out select pieces of their design. Specifically by asking users in interviews, I \DIFdelbegin \DIFdel{will be seeking }\DIFdelend \DIFaddbegin \DIFadd{sought }\DIFaddend to understand why \DIFdelbegin \DIFdel{the users }\DIFdelend \DIFaddbegin \DIFadd{they }\DIFaddend turned to Calico, and how the designs they created were used after the users were finished using Calico.
...
The third group was a distributed software research group located at the University of California, Irvine on the west coast and Carnegie Mellon University on the east coast. In this text, I refer to this group as the research group. This deployment, depicted in Figure \ref{fig:evaluation:setting-researchgroup}, \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{mixed }\DIFaddend existing users of Calico \DIFdelbegin \DIFdel{, and were asked to participate in the qualitative study based on their extensive use of Calico }\DIFdelend \DIFaddbegin \DIFadd{at UC Irvine, and people new to Calico at CMU}\DIFaddend . Two members were located on the west coast and had access to two adjacent Hitachi Starboard FXDUO77 machines, as well as tablet based machines. One member was located on the east coast with access to an HP tablet machine with an electronic pen. This research group collaborated on a research project for a massive online development environment for developing javascript projects. This group was tasked with writing research papers, designing the \DIFdelbegin \DIFdel{frontend}\DIFdelend \DIFaddbegin \DIFadd{front end}\DIFaddend , and writing the software for their project. While they used Calico for a long \DIFdelbegin \DIFdel{peroid }\DIFdelend \DIFaddbegin \DIFadd{period }\DIFaddend of time, only the most recent seven months (a time frame coinciding with the interaction design group) were evaluated. \DIFaddbegin \DIFadd{During that time, the CMU member visited UCI for a week of intense work with Calico; though it saw frequent use thereafter. While apart, skype was used to coordinate their weekly meetings, and was used alongside Calico in sessions.
}\DIFaddend 
...
Based on examinations of logs and recent Calico content, I \DIFdelbegin \DIFdel{periodically }\DIFdelend conducted semi-structured face-to-face interviews. I conduct two semi-structured interview with the OSS group, one with the interaction design group, and one with the research group. In each interview, I began by asking ``What was your most vivid design experience with Calico?'' From this question, participants typically walked through the designs they created in Calico, physically pointing, mimicking their usage of features, and describing the contents of, as well as the activity leading to, their designs. During \DIFdelbegin \DIFdel{design }\DIFdelend \DIFaddbegin \DIFadd{these }\DIFaddend walkthroughs, I asked for clarification of content within their designs, as well as activity that I observed from their usage logs.

In subsequent questions, I asked questions that specifically targeted the first two research questions. First, in order to evaluate \DIFdelbegin \DIFdel{if Calico was }\DIFdelend \DIFaddbegin \DIFadd{whether Calico could be considered }\DIFaddend ``minimally invasive'', I asked participants if they needed to make any compromises in using Calico, \DIFdelbegin \DIFdel{``}\DIFdelend what obstacles or surprises \DIFdelbegin \DIFdel{did you encounter in creating your design'' or }\DIFdelend \DIFaddbegin \DIFadd{they encountered in creating their designs and }\DIFaddend if they struggled with the features. \DIFdelbegin \DIFdel{Further I }\DIFdelend \DIFaddbegin \DIFadd{I also }\DIFaddend inquired how design sessions would have gone had they not used Calico. 
\DIFaddbegin 

\DIFaddend Second, in order to evaluate if Calico has a ``coherent set of features'', I asked participants to report their experience on using the features in general, \DIFdelbegin \DIFdel{``What features did you find helpful''}\DIFdelend \DIFaddbegin \DIFadd{what features they found helpful}\DIFaddend , and if the features clashed with one another. 

Due to the sensitive nature of some of the data collected, measures were taken to ensure that data was securely collected. Of the three sites visited, those in charge of two of the sites \DIFaddbegin \DIFadd{(the OSS group and the research group) }\DIFaddend provided consent to use the data collected for academic publication. \DIFdelbegin \DIFdel{One of the three sites}\DIFdelend \DIFaddbegin \DIFadd{The third site (the interaction design group)}\DIFaddend , however,  was under a strict NDA agreement, and all images, logs, and raw data were not permitted to be taken off-site. In this case, usage logs were processed and reviewed on-site, and only personal notes were permitted to be taken off-site. Furthermore, for the purpose of publication, relevant images containing sensitive content were redrawn such that key insights, such as usage of notations, are preserved, but the content itself is anonymized.

\subsection{Data analysis}

Based on the logs and interviews, collected data from each session was \DIFdelbegin \DIFdel{grouped into the categories pertaining to }\DIFdelend \DIFaddbegin \DIFadd{reviewed in two phases. First, I reviewed interviews for each group for responses pertaining to the overall design activity that occurred, their general feedback on their experiences using Calico, and feedback on specific feature usage. In the second phase, I reviewed both the usage logs and interviews and categorized my observations into the following three categories: 1) }\DIFaddend how they navigated between canvases, \DIFaddbegin \DIFadd{2) }\DIFaddend the representations they used, and \DIFaddbegin \DIFadd{3) }\DIFaddend how they conducted group work. 
\DIFdelbegin \DIFdel{I grouped content into stories of how the design session was conducted, overall feedback on the design activity, and feedback on specific feature usage. 
}\DIFdelend 

%Lastly, the interview notes and logs were examined for any surprises that do not fit within the research question posed.
Additionally, the results from the three field sites is partitioned across \DIFdelbegin \DIFdel{sections }\DIFdelend \DIFaddbegin \DIFadd{Sections }\DIFaddend \ref{chapter:evaluation:deployment1}, \ref{chapter:evaluation:deployment2}, and \ref{chapter:evaluation:deployment3} due to the variety of behaviors observed. The observations across all three sites are instead aggregated in the discussion within Chapter \ref{chapter:discussion}.

\section{Results: Deployment at a commercial open source software company}
\label{chapter:evaluation:deployment1}

Of the entire duration that Calico was installed at the OSS group, the developers reported using Calico in three extensive design sessions\DIFdelbegin \DIFdel{, as well a handful other personal sketches}\DIFdelend . Across all three sessions, the developers were engaged in developing the next version of a healthcare ``message processing'' tool. 

\subsection{Overview of design sessions}

In the first design session, depicted \DIFaddbegin \DIFadd{with excerpts of the canvases }\DIFaddend in Figure \ref{fig:ossgroup:session1}, the developers turned to the whiteboard to sketch out a user interface for creating custom handlers for different types of messages. In the second design session, depicted \DIFaddbegin \DIFadd{with exerpts of the canvases }\DIFaddend in Figure \ref{fig:ossgroup:session2}, the developers turned to the whiteboard to design a set of slides that will be used to explain the software architecture of their system. In this case, all members already understood the architecture, but wanted to create a representation ``that was easy to understand''. In \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend third design session, depicted in Figure \ref{fig:ossgroup:session3} \DIFaddbegin \DIFadd{once more with excerpts}\DIFaddend , while coding, a developer turned to Calico in order to refactor \DIFdelbegin \DIFdel{a set of }\DIFdelend \DIFaddbegin \DIFadd{source }\DIFaddend code that needed to be updated for their upcoming software release.
...
On the whole, the individuals that did use Calico reported that they did not find the features to be invasive to the basic sketching activity. At a bare minimum, all individuals said \DIFaddbegin \DIFadd{they }\DIFaddend were able to use the large electronic whiteboard as they would a regular whiteboard. Individuals reported some drawbacks with the hardware itself, in which the electronic whiteboard hardware experienced lag and needed to write slowly to create accurate representations. However, the pen-based tablets were accurate. They also reported some surprises in the usability of the tool, \DIFdelbegin \DIFdel{but they were always able to fall back to sketching}\DIFdelend \DIFaddbegin \DIFadd{for example they expected the forward and backward navigation buttons to step through the chain of linked canvases in the intentional interface, instead of the visited canvases. Also, the members of the OSS group expected greater functionality in changing colors of scraps and text-scraps, but fell back to plain sketching to use colors}\DIFaddend .

When asked if they made any compromises in order to perform their activities using Calico, users responded that they did not need to make many compromises. \DIFaddbegin \DIFadd{The only compromises that they reported were writing slower and larger on the electronic whiteboard. }\DIFaddend If they did not use Calico, they reported that they would have moved to a meeting room with several whiteboards rather than the space immediately adjacent to their desks. One participant reported that in one activity they would have written on the window using erasable markers, but instead chose to use Calico. They also reported that they normally would have taken pictures of their work, but instead used the email feature of Calico.

\subsubsection{Feature specific}

When asked about the features as a whole, the \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{members of the OSS group }\DIFaddend reported that they did not feel that the features conflicted with one another. Additionally, both interviews and usage logs confirmed that all features were indeed used at least to some extent. The following captures the general feedback from the \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend concerning each feature.

\textbf{Basic sketching and features.} Aside from the issues with the responsiveness of the large electronic whiteboard mentioned above, OSS group \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{members }\DIFaddend reported that the basic sketching features were helpful. Sketching on regular tablets was responsive, and developers \DIFaddbegin \DIFadd{from the OSS group }\DIFaddend stated that synchronous sketching was helpful in meetings, allowing members to work together, or hand tablets back and forth between each other while designing with the large electronic whiteboard. They reported frequent usage of multiple colors and the undo/redo functionality.  They did, however, mistake the ``clear canvas'' button for the ``eraser mode'' button several times, leading to the canvas being accidentally cleared, but this action could be undone with the ``undo'' functionality. Two of the developers \DIFaddbegin \DIFadd{in the OSS group }\DIFaddend reported some difficulty adapting because upon using the device they default to gestures they use on other devices such as pinch-and-zoom, which are not available in Calico. Another \DIFdelbegin \DIFdel{developer }\DIFdelend \DIFaddbegin \DIFadd{member of the OSS group }\DIFaddend requested additional colors be made available as well.

\textbf{Scraps.} The participants found the use of scraps helpful on occasion. One \DIFdelbegin \DIFdel{developer }\DIFdelend \DIFaddbegin \DIFadd{member from the OSS group }\DIFaddend wanted to create text-scraps with different colored text, but instead turned to \DIFdelbegin \DIFdel{sketching }\DIFdelend \DIFaddbegin \DIFadd{writing by hand instead of typing }\DIFaddend when this was not possible. Further, when using text scraps at the large electronic whiteboard, \DIFdelbegin \DIFdel{the developers }\DIFdelend \DIFaddbegin \DIFadd{members from the OSS group }\DIFaddend found it cumbersome to switch between the bluetooth keyboard and the large display, and stated that they would have preferred an on-screen keyboard. Writing content was sloppy, so where possible, they created text-scraps to make content more legible. Otherwise, \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group members }\DIFaddend reported that scraps were sufficiently helpful to move content and represent objects. Among the \DIFdelbegin \DIFdel{five }\DIFdelend \DIFaddbegin \DIFadd{three }\DIFaddend design sessions observed, scraps were used to represent objects in three of those sessions, and in the other two sessions, were used simply to manipulate plain sketched content.  In another design session, one of the \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{members of the OSS group }\DIFaddend encountered difficulty in using the ``shrink-to-contents'' functionality for scraps, which creates a rectangular scrap around sketches if present, or simply a rectangle if no content was present. \DIFaddbegin \DIFadd{The OSS group member reported wanting to create a rectangle, but the ``shrink-to-contents'' button instead created a smaller rectangle around the existing sketch. }\DIFaddend Lastly, a \DIFdelbegin \DIFdel{developer }\DIFdelend \DIFaddbegin \DIFadd{member of the OSS group }\DIFaddend requested that scraps be resized with a corner anchor, without locking their aspect ratio.

\textbf{Palette.} The palette received light usage \DIFdelbegin \DIFdel{from the developers}\DIFdelend \DIFaddbegin \DIFadd{in the OSS group}\DIFaddend . The palette was used in one of the \DIFdelbegin \DIFdel{five }\DIFdelend \DIFaddbegin \DIFadd{three }\DIFaddend design sessions, of which a \DIFdelbegin \DIFdel{representation was }\DIFdelend \DIFaddbegin \DIFadd{few scraps were }\DIFaddend used across several canvases. When asked how often they reused \DIFdelbegin \DIFdel{images, the developers }\DIFdelend \DIFaddbegin \DIFadd{scraps, the OSS group collectively }\DIFaddend replied rarely. They reported that \DIFdelbegin \DIFdel{sketches were created to design a component, after-which }\DIFdelend \DIFaddbegin \DIFadd{they typically went a new blank canvas when discussing their software system, after which }\DIFaddend they archived the \DIFdelbegin \DIFdel{image }\DIFdelend \DIFaddbegin \DIFadd{canvases used }\DIFaddend by emailing it to themselves. With respect to reusing \DIFdelbegin \DIFdel{images }\DIFdelend \DIFaddbegin \DIFadd{scraps }\DIFaddend with the palette, they stated that it was faster to simply redraw \DIFdelbegin \DIFdel{the image}\DIFdelend \DIFaddbegin \DIFadd{their representations}\DIFaddend .

\textbf{Intentional interfaces.} \DIFdelbegin \DIFdel{Developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group members }\DIFaddend responded that they found the ``chaining''\DIFaddbegin \DIFadd{, i.e., linking canvases using tags, }\DIFaddend created by intentional interfaces to be helpful in capturing individual design sessions. They used the ``new canvas'' and ``copy canvas'' buttons several times with tagging to create chains, and all design activities were done in a single cluster. When drawing within a canvas, one \DIFdelbegin \DIFdel{developer }\DIFdelend \DIFaddbegin \DIFadd{OSS group member }\DIFaddend reported that they exclusively used the breadcrumb bar to navigate between canvases. Another developer \DIFaddbegin \DIFadd{from the OSS group }\DIFaddend preferred to use the cluster view, but reported being annoyed that the cluster view \DIFdelbegin \DIFdel{did }\DIFdelend changed their zoom-perspective in an unexpected ways. In their words, the view ``kept jumping around''. As a result, the users reported a lot of unnecessary panning in the cluster view. Two other developers \DIFaddbegin \DIFadd{in the OSS group }\DIFaddend also reported that they found the forward and \DIFdelbegin \DIFdel{back }\DIFdelend \DIFaddbegin \DIFadd{backward }\DIFaddend navigation buttons confusing. They expected these buttons to navigate to the next canvas in the chain within the intentional interface, not the most recently visited canvas. \DIFdelbegin \DIFdel{Wanted back and forth step in navigation history, and back and forth in the chain. One developer }\DIFdelend \DIFaddbegin \DIFadd{One developer in the OSS group }\DIFaddend asked for a minimap of the clusterview from the canvas. One \DIFdelbegin \DIFdel{developer }\DIFdelend \DIFaddbegin \DIFadd{member of the OSS group }\DIFaddend called the organization ``wizardry'', since they could not see how it \DIFdelbegin \DIFdel{changes}\DIFdelend \DIFaddbegin \DIFadd{changed}\DIFaddend .

\textbf{Fading highlighter.} \DIFdelbegin \DIFdel{Developers }\DIFdelend \DIFaddbegin \DIFadd{The OSS group }\DIFaddend reported that the fading highlighter was useful within group sessions. They used the fading highlighter in two of the five sessions, and reported that they used this feature to explain designs.

\subsection{Canvas navigation}

\textbf{Using intentional interface chains to capture design sessions.} The \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend reported that they organized each design session by ``chaining'' canvases within the cluster view\DIFaddbegin \DIFadd{, as depicted in Figure \ref{fig:ossgroup:clusterview}}\DIFaddend . Each design session was linked together using tags, using either the ``perspective'' tag or the ``alternative'' tag. Each new design session began as a new canvas within the center of the radial circle, and extended outward using canvases linked \DIFdelbegin \DIFdel{with connectors. A developer }\DIFdelend \DIFaddbegin \DIFadd{as chains. A member of the OSS group }\DIFaddend reported that panning through their chains of canvases was helpful in reviewing the sketches that they created.

\DIFaddbegin \begin{figure}%DIF > 
  \centering
  \includegraphics[width=12cm,keepaspectratio]{./figures/Evaluation/ossgroup/ossgroup-clusterview}
   \caption {\DIFaddFL{Cluster view of canvases. The three major sessions were grouped by ``chaining'' canvases.}}
   \label{fig:ossgroup:clusterview}   
\end{figure}%DIF > 

\DIFaddend \textbf{Using canvases to explore perspectives.} The \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend reported that they used several canvases to explore their designs across multiple perspectives. They explored different perspectives both by using different types of representations, such as user interface mockups and software architecture diagrams, and also by creating sketches that \DIFdelbegin \DIFdel{depict different pieces }\DIFdelend \DIFaddbegin \DIFadd{depicted different parts }\DIFaddend of the system. 
...
In their second design session, depicted in Figure \ref{fig:ossgroup:session2}, they \DIFdelbegin \DIFdel{depict different pieces }\DIFdelend \DIFaddbegin \DIFadd{explored different parts }\DIFaddend of the same design across the different canvases in order document the flow of information through the system. For example, Figure \ref{fig:ossgroup:session2:a} shows ``message processing'' sending information to the ``Event Bus''. Figure \ref{fig:ossgroup:session2:c} shows how information is passed from the ``Event Bus'' scrap onto the ``Event Listener'' scraps. Figure \ref{fig:ossgroup:session2:d} further shows how information is passed from the ``Alert'' scrap, which itself is an instantiation of an ``Event Listener'', onto the ``Email'' and ``Channel'' scraps. The \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group members }\DIFaddend sketched out each step in order to thoroughly explain the rules and logic used for passing information between components. 

\textbf{Stepping through a design using levels of abstraction. } The \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend used multiple levels of abstraction to aid \DIFdelbegin \DIFdel{them }\DIFdelend in explaining their architecture. They used it both to progressively step into the detail of their design, and also to back away to once again see the bigger picture.

When stepping progressively \DIFdelbegin \DIFdel{stepping }\DIFdelend into their design, they made use of the ``copy canvas'' feature to repeat a canvas, but replace \DIFdelbegin \DIFdel{pieces }\DIFdelend \DIFaddbegin \DIFadd{parts }\DIFaddend of their sketches with more detailed versions \DIFaddbegin \DIFadd{to step into lower levels of abstraction}\DIFaddend . The second design session in Figure \ref{fig:ossgroup:session2} shows such a case. \DIFaddbegin \DIFadd{First, Figure \ref{fig:ossgroup:session2:b} shows a lower level of abstraction of Figure \ref{fig:ossgroup:session2:a}. }\DIFaddend Between Figures \ref{fig:ossgroup:session2:a} and \ref{fig:ossgroup:session2:b}, ``message processing'' is replaced by \DIFaddbegin \DIFadd{scraps labeled as }\DIFaddend ``Channel'' \DIFdelbegin \DIFdel{scraps}\DIFdelend , which represent the components that process the messages.  \DIFdelbegin \DIFdel{The generic black connectors are also replaced with colored connectors to depict that messages of specific types are passed to }\DIFdelend \DIFaddbegin \DIFadd{In Figure \ref{fig:ossgroup:session2:b}, each colored connector represents messages of a specific type that are passed from a ``Channel'' to the }\DIFaddend ``Event Bus''. \DIFdelbegin \DIFdel{Within Figure \ref{fig:ossgroup:session2:c}, the }\DIFdelend \DIFaddbegin \DIFadd{Second,  Figures \ref{fig:ossgroup:session2:c}, \ref{fig:ossgroup:session2:d}, \ref{fig:ossgroup:session2:e}, and \ref{fig:ossgroup:session2:f} show a sequential step-through in levels of abstraction. The }\DIFaddend ``Event Listener'' scrap \DIFdelbegin \DIFdel{expanded upon using }\DIFdelend \DIFaddbegin \DIFadd{in Figure \ref{fig:ossgroup:session2:c} is represented as }\DIFaddend the ``Alert'' scrap \DIFdelbegin \DIFdel{, which instantiates ``Event Listener'', in both Figures \ref{fig:ossgroup:session2:d}and \ref{fig:ossgroup:session2:e}. The developers further progressing define ``Action Group'' , where the respective component is defined as }\DIFdelend \DIFaddbegin \DIFadd{in Figure \ref{fig:ossgroup:session2:d}, and is shown in much greater detail. The }\DIFaddend ``AG1'' \DIFdelbegin \DIFdel{and ``AG2'' within Figure \ref{fig:ossgroup:session2:d}, defined further with three elements }\DIFdelend \DIFaddbegin \DIFadd{scrap in Figure \ref{fig:ossgroup:session2:d} is represented as the ``Action Group'' scrap in Figure \ref{fig:ossgroup:session2:e}, and also elaborated in much more detail. The ``Action Group'' scrap }\DIFaddend in Figure \ref{fig:ossgroup:session2:e} \DIFdelbegin \DIFdel{, and expanded upon further in \ref{fig:ossgroup:session2:f}}\DIFdelend \DIFaddbegin \DIFadd{is again represented in ``Action Group'' in Figure \ref{fig:ossgroup:session2:f}, showing further sub components for ``Action Group''}\DIFaddend . While creating these scraps, the \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend used the palette to copy elements between canvases, such as \DIFdelbegin \DIFdel{with }\DIFdelend ``Event Bus'' and ``Action Group''.

After their stepwise exploration leading to Figure \ref{fig:ossgroup:session2:f}, the \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group members }\DIFaddend sketched a high level picture of messages passing through the system in Figure \ref{fig:ossgroup:session2:h}. Not \DIFdelbegin \DIFdel{depicted, the developers }\DIFdelend \DIFaddbegin \DIFadd{shown in the figure, the OSS group members }\DIFaddend created several copies of the contents \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend Figure \ref{fig:ossgroup:session2:h} in order to explain how different types of messages may pass through the system. In their session, they returned to previous canvases when they had questions about particular components.

\textbf{Copying canvases to generate alteratives. } The \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend sometimes used multiple canvases to create alternative solutions to existing designs. For example, a \DIFdelbegin \DIFdel{developer reported that}\DIFdelend \DIFaddbegin \DIFadd{member of the OSS group reported that, }\DIFaddend while working with their team members on the user interface in Figure \ref{fig:ossgroup:session1:c}, they ``did not quite agree with the design'' \DIFaddbegin \DIFadd{and }\DIFaddend deviated from the group to create their own version. To do so, they used the ``copy canvas'' button, selected the ``alternative'' tag in the intentional interface tag panel, and \DIFdelbegin \DIFdel{create }\DIFdelend \DIFaddbegin \DIFadd{created }\DIFaddend a new user interface, the result of which is shown in  \ref{fig:ossgroup:session1:d}. 

In a third design session, one of the \DIFaddbegin \DIFadd{OSS group }\DIFaddend developers used multiple canvases to help them in iterating on past designs. After finishing the design in Figure \ref{fig:ossgroup:session3:a}, the \DIFdelbegin \DIFdel{developer }\DIFdelend \DIFaddbegin \DIFadd{OSS group member actually }\DIFaddend implemented the designed changes \DIFdelbegin \DIFdel{, and later returned }\DIFdelend \DIFaddbegin \DIFadd{in the software to later return }\DIFaddend to the large electronic whiteboard to \DIFdelbegin \DIFdel{continue a second iteration. In their }\DIFdelend \DIFaddbegin \DIFadd{verify their source code correctly implemented the original pseudocode in Calico. In this }\DIFaddend second iteration, they created a copy of \DIFdelbegin \DIFdel{their }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend original canvas and pasted a screenshot of the source code that resulted from the previous design session, where they continued to \DIFdelbegin \DIFdel{iterate on their design}\DIFdelend \DIFaddbegin \DIFadd{refine the pseudocode}\DIFaddend .

\subsection{Representations}

The \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend created several types of representations in their design sessions.

\textbf{\DIFdelbegin \DIFdel{Box and arrow }\DIFdelend \DIFaddbegin \DIFadd{Box-and-arrow }\DIFaddend diagrams. } Across the design sessions, they created \DIFdelbegin \DIFdel{two types of }\DIFdelend \DIFaddbegin \DIFadd{what they termed ``}\DIFaddend box-and-arrow\DIFdelbegin \DIFdel{diagrams. }\DIFdelend \DIFaddbegin \DIFadd{'' diagrams. They did not make any direct reference to notations such as UML class diagrams or ER notation, but rather viewed their sketches as rudimentary abstractions of their existing software system. }\DIFaddend The first was used for brainstorming, and the second for explaining an architecture.

In the first design session\DIFaddbegin \DIFadd{, }\DIFaddend depicted in \ref{fig:ossgroup:session1:a}, they created a set of box-and-arrow diagrams to first brainstorm the components of a user interface. The design session began by first listing out the relevant entities in the software, which were the text-scraps, ``Source'' and ``Destinations''. In this session, the \DIFdelbegin \DIFdel{developer }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend experimented with different combinations of actions, which were reflected with further text-scraps associated with connectors at the bottom. They used connectors to represent the data sources that the `Source'' and ``Destinations'' scraps could pull from and send to.

In the second design session, the \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend used a combination of scraps, connectors, and plain sketches to represent software components and the passing of data. When asked why they sometimes represented components as plain sketches \DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{and other times }\DIFaddend using scraps, they reported that they used scraps to represent components that they were actively designing and using. The \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend reported that elements in sketches, such as ``In'' and ``Out'' in Figure \ref{fig:ossgroup:session2:a} and LLP in \ref{fig:ossgroup:session2:h}, represented outside information. Creating items such as ``Event Listener'' in Figure \ref{fig:ossgroup:session2:c} allowed the components to be reused and moved. For passing data, they simply used colors to represent particular types of data that was passed. For example, between Figures \ref{fig:ossgroup:session2:a} and \ref{fig:ossgroup:session2:b}, the \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend used four different types of colors \DIFaddbegin \DIFadd{to }\DIFaddend show the types of data that \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend passed from a ``Channel'' to the ``Event Bus''. In Figure \ref{fig:ossgroup:session2:c}, the \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend repeated the use of colors to show how the different types of data were handled.

\textbf{User Interface Mockups. } Across the sketches in Figure \ref{fig:ossgroup:session1}, the \DIFdelbegin \DIFdel{developer }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend used a mix of plain sketching with scraps to design a component and its interface across several canvases. Within both Figure \ref{fig:ossgroup:session1:a} and \ref{fig:ossgroup:session1:b}, \DIFdelbegin \DIFdel{the developer sketched both the }\DIFdelend \DIFaddbegin \DIFadd{they sketched }\DIFaddend the software entities alongside a user interface mockup. \DIFdelbegin \DIFdel{Within }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend Figure \ref{fig:ossgroup:session1:a}, the \DIFdelbegin \DIFdel{developer }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend reported that they created three low-detailed user interface mockups while ``throwing ideas out on the board'' to represent the entities in the box-and-arrow diagrams. They iterated over their user interface across multiple canvases, some of which are depicted in Figures \ref{fig:ossgroup:session1:c} and \ref{fig:ossgroup:session1:b}.
...
\textbf{Lists. } The \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend used lists in two instances. First they used lists as a method to hierarchical organize the user interface in Figure \ref{fig:ossgroup:session1:b}. Second, they used lists as a mechanism to describe the functionality of a software architecture in detail in Figure \ref{fig:ossgroup:session2:g}. The \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend reported that they used lists to summarize their design sessions and to reflect on designs they recently created. However, they did not create many lists because they found that the board reduced the aesthetic quality of their writing.

\textbf{Use case sketches. } In order to capture the behavior of their designs at runtime, they created primitive use case scenarios. For example, in the final canvas of their first design session, Figure \ref{fig:ossgroup:session1:e}, the \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend sketched the behavior of the user interface when different channels were checked. This \DIFdelbegin \DIFdel{final }\DIFdelend representation depicts the results of three different use cases for the interfaces shown in Figures \ref{fig:ossgroup:session1:c} and \ref{fig:ossgroup:session1:d}. The left scrap in Figure \ref{fig:ossgroup:session1:e} represents the input panel, and the three scraps on the right hand side each represent the \DIFdelbegin \DIFdel{results of different use cases}\DIFdelend \DIFaddbegin \DIFadd{result of a different use case}\DIFaddend , in which either ``Ch 1'', ``Ch 2'', or ``Ch 3'' \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend tapped on the left scrap. The \DIFdelbegin \DIFdel{developer }\DIFdelend \DIFaddbegin \DIFadd{member from the OSS group }\DIFaddend created the three previews by drafting an initial scrap on the right hand side, and copying it multiple times, adjusting the values for each respective ``Channel'' selected. 

\DIFdelbegin \DIFdel{Within }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend the second design session, they created several copies of the contents in Figure \ref{fig:ossgroup:session2:h} to explore different use case scenarios. They used different use case scenarios to walk through input messages of different types and frequency, as well as using different event listeners to handle those messages.

\textbf{Source code. } In the third design session, one of the \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{members of the OSS group }\DIFaddend used the large electronic whiteboard to help him in refactoring his code. Depicted in Figure \ref{fig:ossgroup:session3}, the \DIFdelbegin \DIFdel{developer }\DIFdelend \DIFaddbegin \DIFadd{OSS group member }\DIFaddend needed to refactor his code in order to process an XML file that contains new fields in the next version of their software tool. In order to carry out his design session, he opened Calico on his own desktop, copy and pasted a screenshot of the XML code that he needed to process, and continued his design session at the large electronic whiteboard. In his exit interview, he needed ``a space to think freely''. While sketching, he used several custom annotations and colors to depict the process flow of different software components through the XML structure. Black vertical bars were used to represent a call stack, which he drew to help him in understanding how far the callstack goes in a recursive call. The red arrows were used to represent how the call stack moves through the data.

Overall, the \DIFdelbegin \DIFdel{developer }\DIFdelend \DIFaddbegin \DIFadd{OSS group member }\DIFaddend reported that using different colors helped him see things ``at-a-glance''. \DIFdelbegin \DIFdel{They stepped back their design }\DIFdelend \DIFaddbegin \DIFadd{He stepped back from his pseudocode }\DIFaddend several times, experimented with walking through the code in different orders, \DIFdelbegin \DIFdel{until settling on the design shown }\DIFdelend \DIFaddbegin \DIFadd{and used a different color for each walkthrough. The final design of the first iteration isshown }\DIFaddend in Figure \ref{fig:ossgroup:session3:a}. After implementing the design \DIFdelbegin \DIFdel{, the develop copy }\DIFdelend \DIFaddbegin \DIFadd{in the code base, and upon discovery that an issue was not addressed in his first iteration, the OSS group member copied }\DIFaddend and pasted the \DIFdelbegin \DIFdel{resulting }\DIFdelend \DIFaddbegin \DIFadd{latest }\DIFaddend Java source code to \DIFaddbegin \DIFadd{once more }\DIFaddend step through their design again to \DIFdelbegin \DIFdel{create a second iteration}\DIFdelend \DIFaddbegin \DIFadd{address the discovered issue}\DIFaddend .

\subsection{Collaborative work}

\textbf{Preparing work ahead of time.} The first design session was a shared group session that spanned the greater part of a working day. In the morning, one of the \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group members }\DIFaddend took a tablet to their desk and worked out a design, and later presented his design in a group meeting with four other developers \DIFaddbegin \DIFadd{in the OSS group}\DIFaddend , where they continued to work together on the design for several hours afterwards. A subset of the sketches produced in this design session are depicted in Figure \ref{fig:ossgroup:session1}. In this design session, they were designing how to handle data as it is passed through each ``channel''.

\textbf{Explaining designs to the group.} In the second design session, the team members made heavy use of multiple tablets, as well as the fading highlighter. When the group meeting began in the afternoon, the \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group }\DIFaddend made heavy use of the fading highlighter to explain the behavior of the system within Figure \ref{fig:ossgroup:session2:a}. Having \DIFdelbegin \DIFdel{independently }\DIFdelend worked out much of the design independently earlier that day, a \DIFdelbegin \DIFdel{developer }\DIFdelend \DIFaddbegin \DIFadd{member of the OSS group }\DIFaddend used the fading highlighter to explain the state of the design thus far. 

\textbf{Spontaneous asynchronous work.} During the design session, some of the design members would privately move to another cell, create a design, and call other members of the group to visit their cell. For example, \DIFdelbegin \DIFdel{While }\DIFdelend \DIFaddbegin \DIFadd{while }\DIFaddend the group discussed the data that was passed from ``Event Bus'' in Figure \ref{fig:ossgroup:session2:d}, one of the \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{OSS group memebers }\DIFaddend wanted to understand how the ``Alert'' component worked in the greater context, and moved to the canvas depicted in Figure \ref{fig:ossgroup:session2:h} to sketch a much more abstract representation to show how data flowed through it. After sketching it, they called the other designers over, and made frequent movements back and forth between these canvases to compare them.
...
With Calico, they saw an opportunity to perform the same activity, but preserve work that would have been lost every time they re-arranged the note cards. Instead, they performed this activity by importing images of the fifty individuals that they interviewed into the palette, and dropped these onto the canvas as \DIFdelbegin \DIFdel{image-scraps}\DIFdelend \DIFaddbegin \DIFadd{image scraps}\DIFaddend . In Calico, they viewed the work they did as \DIFdelbegin \DIFdel{reusable}\DIFdelend \DIFaddbegin \DIFadd{non-destructive and preservative}\DIFaddend , where an organization could be copied and reorganized an indefinite amount of times. If inspiration struck them at a later moment, they were always able to return to a previous configuration of \DIFdelbegin \DIFdel{image-scraps }\DIFdelend \DIFaddbegin \DIFadd{image scraps }\DIFaddend and continue with \DIFdelbegin \DIFdel{their }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend most recent work \DIFaddbegin \DIFadd{later}\DIFaddend .
...
Additionally, the interaction designers had several feature requests. They found the use of modal dialogs, such as email confirmation dialogs and text entry, to be \DIFdelbegin \DIFdel{disrupting }\DIFdelend \DIFaddbegin \DIFadd{disruptive }\DIFaddend during their design sessions. They asked for highlighting functionality that is more analogous to its real-life counter part, \DIFaddbegin \DIFadd{i.e., persistent and shades the area behind a text with color, }\DIFaddend which they reported as using often for tagging items in their sketches. They further reported the sudden switch between sketching an object, and creating scraps to be disorienting, and requested separate modes for drawing, selecting and moving scraps, and creating scraps. They further found the set of graphical icons for buttons and menus to not be straightforward, and requested additional descriptions.

\DIFaddbegin \DIFadd{Despite these issues, they produced twenty one canvases of content. Sixteen of those canvases made significant use of scraps, six of which used made use of multiple list-scraps. One canvas in total used formal connectors between scraps.
}

\DIFaddend \subsubsection{Feature specific}

\textbf{Basic sketching and features.} Participants reported some difficulty in accurately sketching with the system. \DIFaddbegin \DIFadd{They reported that it was difficult to produce legible handwriting. For example, when attempting to use printing, the system would produce cursive text. Other times, the system would lag and sometimes lose details in their writing.
}\DIFaddend 

\textbf{Scraps.} The participants reported that the \DIFdelbegin \DIFdel{scraps that the }\DIFdelend functionality that scraps enabled, such as moving sketched contents\DIFaddbegin \DIFadd{, }\DIFaddend was helpful, but reported some difficulty in using scraps. The participants reported that the interaction for moving scraps felt slow, particularly when moving scraps to categorize them in tables or quadrants. Further, the system began to slow down when using a large number of \DIFaddbegin \DIFadd{image }\DIFaddend scraps, thirty or more, within the same canvas. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{This occurred in other canvases when the }\DIFaddend participants requested a specialized mode for moving and arranging scraps quickly.

\DIFaddbegin \DIFadd{It is interesting to note, though, that early versions of Calico Version Two had such separate modes, which was received with very mixed reactions when deployed to students in a software design class. Students frequently had the mode switch problem: they did not know mode they were using. This was one of the reasons we reduced the number of modes dramatically.
}

\DIFaddend \textbf{Palette.} The participants found the palette helpful for loading pre-existing graphics. The participants imported all fifty images of the people they interviewed into the palette, and used the images from the palette across several canvases. However, they found it difficult to find items in the palette when they loaded all fifty images into it\DIFaddbegin \DIFadd{, as the palette displays saved scraps as small snapshots that are difficult to discern}\DIFaddend .

\textbf{Grid / Intentional interfaces.} During the majority of their usage, the interaction designers did not have intentional interfaces, but instead had the grid. They reported that they often moved to the grid perspective in order to get a high level perspective of their work. One of the designers reported that ``it's important for me to see everything at once''. The interaction designers used intentional interfaces for a brief period, but had difficulty adjusting to using it as a method of moving between canvases. They had trouble using the breadcrumb bar to navigate between canvases, and found the preview of the canvases within the cluster view too small. \DIFaddbegin \DIFadd{This, in a way, is not surprising given the brief exposure.
}\DIFaddend 

\textbf{Fading highlighter.} The interaction designers reported positive feedback about this feature, however\DIFaddbegin \DIFadd{, they }\DIFaddend seldomly used it in practice. They reported the feature would be \DIFdelbegin \DIFdel{highlighly }\DIFdelend \DIFaddbegin \DIFadd{highly }\DIFaddend useful in working with larger groups, but all of their sessions took place with at most two interaction designers. Further, they found the name confusing as it did not function as they expected a regular highlighter would, in which a marker changes the color behind written text. Instead, they colloquially referred to it as the ``John Madden mode'', in reference to the sports announcer.
...
\textbf{Multi-dimensional axis.} In Figures \ref{fig:ixdgroup:session1:e} and \ref{fig:ixdgroup:session1:f}, the interaction designers used a two dimensional axis to further organize their \DIFdelbegin \DIFdel{image-scraps}\DIFdelend \DIFaddbegin \DIFadd{image scraps}\DIFaddend . Within the sessions performed in the canvases depicted in both figures, the interaction designers first began with a one-dimensional axis that plotted a range of behaviors within a particular quality. They then placed the images and names of the individuals they interviewed along this axis. Afterward, they converted the sketch into a two dimensional axis by drawing a line down the middle, and then further grouping the images along the new axis. Within the canvas depicted in Figure \ref{fig:ixdgroup:session1:f}, they juxtaposed \DIFdelbegin \DIFdel{another set of grouping, but with three categories instead of four by using a triangle}\DIFdelend \DIFaddbegin \DIFadd{a three-dimensional triangle axis next to a two-dimensional axis, both of which categorized a different set of scraps}\DIFaddend .

\textbf{Organization by axis, tags, and \DIFdelbegin \DIFdel{euler }\DIFdelend \DIFaddbegin \DIFadd{Euler }\DIFaddend notation.} \ref{fig:ixdgroup:session1:b} After copying scraps onto the canvas, they used several techniques to \DIFdelbegin \DIFdel{simulataneously }\DIFdelend \DIFaddbegin \DIFadd{simultaneously }\DIFaddend overlay  different layers of categorization. Within Figure \ref{fig:ixdgroup:session1:b}, the \DIFdelbegin \DIFdel{developers }\DIFdelend \DIFaddbegin \DIFadd{interaction designers }\DIFaddend used three types of categorization. First, they used a table. When the designers first entered this canvas, they initially dropped \DIFdelbegin \DIFdel{image-scraps }\DIFdelend \DIFaddbegin \DIFadd{image scraps }\DIFaddend of those that they interviewed onto the canvas to view, and immediately afterwards drew a one-dimensional line and partitioned it into segments, similar to the canvas in Figure \ref{fig:ixdgroup:session1:a}. This time, however, they extended the one dimensional line into a table by drawing long vertical lines and categorized the scraps within these spaces. Second, they used \DIFdelbegin \DIFdel{euler }\DIFdelend \DIFaddbegin \DIFadd{Euler }\DIFaddend diagram notation to depict emergent groupings by circling a set of \DIFdelbegin \DIFdel{image-scraps }\DIFdelend \DIFaddbegin \DIFadd{image scraps }\DIFaddend and writing the name of the category next to the grouping. For example, the two \DIFdelbegin \DIFdel{image-scraps }\DIFdelend \DIFaddbegin \DIFadd{image scraps }\DIFaddend in the top left have a yellow circle around them with the text ``NO CONDITION''. Third, they used color tags to denote another level of grouping. On the far left side, they created a legend with mappings between color and a category name. They then tagged the \DIFdelbegin \DIFdel{image-scraps }\DIFdelend \DIFaddbegin \DIFadd{image scraps }\DIFaddend with color. The result was a set of \DIFdelbegin \DIFdel{image-scraps }\DIFdelend \DIFaddbegin \DIFadd{image scraps }\DIFaddend that were organized using a table, \DIFdelbegin \DIFdel{euler }\DIFdelend \DIFaddbegin \DIFadd{Euler }\DIFaddend diagrams, and color tags.

\textbf{Spatially clustering objects.} In Figure \ref{fig:ixdgroup:session1:c}, the designers allowed the categories to emerge from the individuals they interviewed in a \DIFdelbegin \DIFdel{bottom up }\DIFdelend \DIFaddbegin \DIFadd{bottom-up }\DIFaddend fashion. In this canvas, they first wrote the topic of the canvas, which was ``Car Shopping Behaviors'', and grouped the faces of the people interviewed into categories based on their experiences buying cars, and then wrote the name of the grouping, such as ``LOVE IT'' or ``BAD EXPERIENCE''. The interaction designers first wrote the topics using the regular pen, then later converted the topics into text-scraps. The interaction designers further copy and pasted their notes from that session, and annotated with a dash to indicate that it was an attribute of a cluster. For example, one text-scrap had the text ``- checked their smart during purchase''. The interaction designers similarly spatially clustered their \DIFdelbegin \DIFdel{image-scraps }\DIFdelend \DIFaddbegin \DIFadd{image scraps }\DIFaddend in Figure \ref{fig:ixdgroup:session1:d}, but additionally used \DIFdelbegin \DIFdel{connectors }\DIFdelend \DIFaddbegin \DIFadd{arrows, and in one case formal connectors between scraps, }\DIFaddend to associated an image scrap with more than one group.

\textbf{Story-driven flowchart. } In one particular instance, the interaction designers walked through the customer's experience by drawing a story, as in Figure \ref{fig:ixdgroup:session1:d}. From the stories they gathered in their \DIFdelbegin \DIFdel{interviewed}\DIFdelend \DIFaddbegin \DIFadd{interview}\DIFaddend , they drew the different events that \DIFdelbegin \DIFdel{occured in }\DIFdelend \DIFaddbegin \DIFadd{occurred during }\DIFaddend an average customer's purchasing experience, and noted possible options that may occur at each point. In order to construct the story, they reused elements they had stored in the palette, such as the image of a stick figure to indicate actors, and various icons to represent events, such as a phone to call a customer after a pending order was fulfilled. For particularly interesting events and corner cases in the story, the interaction designers placed the image of the person they interviewed to add to the story. 
...
%DIF < Original design predated calico. Originally had it as a sketch in his office. ``What kind of state machine was this?''. Ben towne came to town... they had the design machine, but it wasn't really worked out or designed in any sense. The motivation was that they wanted to sit down and work out the details of the state machine once and for all. The design was their effort to sit through all the different sort of states that could be there. All the transitions. They spent a few days doing edits on their diagrams. Canvas1 was a state machine for functions. They have another state machien for tests. The code already existed... so all through the week they would look back at the diagram, and figure out what was implemented and what wasn't.
%DIF > Original design predated calico. Originally had it as a sketch in his office. ``What kind of process flow diagram was this?''. Ben towne came to town... they had the design machine, but it wasn't really worked out or designed in any sense. The motivation was that they wanted to sit down and work out the details of the process flow diagram once and for all. The design was their effort to sit through all the different sort of states that could be there. All the transitions. They spent a few days doing edits on their diagrams. Canvas1 was a process flow diagram for functions. They have another state machien for tests. The code already existed... so all through the week they would look back at the diagram, and figure out what was implemented and what wasn't.
%
...
One of the members reported that it served as a lightweight thinking tool for \DIFdelbegin \DIFdel{them}\DIFdelend \DIFaddbegin \DIFadd{him}\DIFaddend . The third researcher reported that he typically used lightweight \DIFdelbegin \DIFdel{notetaking }\DIFdelend \DIFaddbegin \DIFadd{note-taking }\DIFaddend tools such as \DIFdelbegin \DIFdel{evernote}\DIFdelend \DIFaddbegin \DIFadd{Evernote}\DIFaddend , which helped his individual thinking process, and later shared his work with the group. \DIFaddbegin \DIFadd{He reported that Calico offered more flexibility in what he could create, and found the flexibility helpful in preparing content before sharing in group meetings.
}\DIFaddend 
...
\textbf{Basic sketching and features.} The researchers reported that \DIFdelbegin \DIFdel{basic sketching }\DIFdelend \DIFaddbegin \DIFadd{the basic sketching features }\DIFaddend were adequate for their design sessions, although they reported that the electronic whiteboard made their handwriting messy. They further reported that some of the menu buttons were not clear in their functionality. They expected the ``navigate backwards'' and ``navigate forwards'' buttons to move back and forth in links within the intentional interface rather than navigating the history of visited canvases. They further thought the ``clear canvas'' button was the eraser mode button, causing them to accidentally clear their canvas, which they recovered by pressing ``undo''.

\textbf{Scraps.} The \DIFdelbegin \DIFdel{researcher }\DIFdelend \DIFaddbegin \DIFadd{researchers }\DIFaddend made frequent use of scraps across their design sessions. They found scraps and connectors useful because both made otherwise static elements interactive, such as when working with \DIFdelbegin \DIFdel{state diagrams}\DIFdelend \DIFaddbegin \DIFadd{process flows}\DIFaddend . They reported feeling less afraid to create complex diagrams because the elements could be moved.  They could create space in \DIFdelbegin \DIFdel{state diagrams }\DIFdelend \DIFaddbegin \DIFadd{process flows }\DIFaddend by moving scraps, and also categorizing objects, such as when placing them in tables, like in Figure \ref{fig:researchgroup:c}. 
...
\textbf{Fading highlighter.} The members made light use of fading highlighter. They reported that, while it was useful on occasion, they \DIFdelbegin \DIFdel{prefered }\DIFdelend \DIFaddbegin \DIFadd{preferred }\DIFaddend to talk out loud and make pointing gestures using their hands. \DIFaddbegin \DIFadd{In remote sessions, the research from CMU relied on video communication to see these non-verbal cues. They found it helpful, but often times forgot that it was available. Reported that they would have used it more if it had been easier to invoke.
}\DIFaddend 

\subsection{Canvas navigation}

During sessions involving the design of the \DIFdelbegin \DIFdel{state diagram }\DIFdelend \DIFaddbegin \DIFadd{process flow }\DIFaddend in Figure \ref{fig:researchgroup:a}, the researchers reported that they seldomly moved to another canvas. Rather, they used additional tools such as Google Docs \DIFaddbegin \DIFadd{to record requirements and design decisions, }\DIFaddend or simply talked out loud. \DIFaddbegin \DIFadd{All sessions using Calico using Calico also involved the use of Skype and Google Docs.
}\DIFaddend 

\textbf{Using perspectives to support onboarding.} The researcher that underwent onboarding used canvases of several perspectives in becoming acquainted with the software system. This researcher used a mix of user interface mockups, freehand sketches, tables, and other diagrams to step through their \DIFdelbegin \DIFdel{designs. Their }\DIFdelend \DIFaddbegin \DIFadd{implementation of the software already written and design of new their new features. This }\DIFaddend initial exploration involved importing an image of the interface into Calico,  and stepping through the interface to inspect items pertaining to debugging user generated code. From these sketches, \DIFdelbegin \DIFdel{they }\DIFdelend \DIFaddbegin \DIFadd{he }\DIFaddend switched to other canvases in which he generated the \DIFdelbegin \DIFdel{state diagram }\DIFdelend \DIFaddbegin \DIFadd{process flow }\DIFaddend in Figure \ref{fig:researchgroup:b}, which was another perspective of Figure \ref{fig:researchgroup:a}, but distilled to only include details relevant to his project. Later, while \DIFdelbegin \DIFdel{exploring the state diagram }\DIFdelend \DIFaddbegin \DIFadd{again exploring the process flow }\DIFaddend in Figure \ref{fig:researchgroup:c}, the researcher created several copies of the canvas, preserving the \DIFdelbegin \DIFdel{state diagram }\DIFdelend \DIFaddbegin \DIFadd{process flow }\DIFaddend at the top of the canvas, and creating different tables, lists, and mock-ups below while experimenting with different algorithms.

\textbf{Exploring alternative \DIFdelbegin \DIFdel{state machines}\DIFdelend \DIFaddbegin \DIFadd{process flows}\DIFaddend .} The same researcher that underwent onboarding \DIFdelbegin \DIFdel{further used several canvases to explore alternative designs for building his component. The researcher used the top state diagram in Figure \ref{fig:researchgroup:c} as the stable set of components to build on top of , and design the actions his component would take below, and in the case of }\DIFdelend \DIFaddbegin \DIFadd{drew several alternatives for his part of the system. This researcher created several copies of the canvas in }\DIFaddend Figure \ref{fig:researchgroup:c}, \DIFdelbegin \DIFdel{using }\DIFdelend \DIFaddbegin \DIFadd{which consisted of a process flow diagram juxtaposed against }\DIFaddend a table. \DIFdelbegin \DIFdel{The researcher performed a sequential exploration, in which they designed the first component, then attempted a new solution in a different canvas . They returned to the previous canvas to reference their previous work}\DIFdelend \DIFaddbegin \DIFadd{Between these alternatives, the process flow depicted at the top of the canvas did not change. Instead, the table below served as a template to experiment with alternative paths for components. The researcher frequently referenced his previous canvases to compare against other alternatives}\DIFaddend .
...
The researchers primarily used Calico to work on \DIFdelbegin \DIFdel{state diagrams}\DIFdelend \DIFaddbegin \DIFadd{process flows}\DIFaddend , however they additionally used other types of representations.
...
When the researchers described the diagram, they saw it not as a large mess, but rather a set of higher level structures that represented different \DIFdelbegin \DIFdel{work-flows}\DIFdelend \DIFaddbegin \DIFadd{parts of a complex work flow}\DIFaddend . Their discussions took place over \DIFdelbegin \DIFdel{these particular }\DIFdelend \DIFaddbegin \DIFadd{different }\DIFaddend regions of the diagram, which the designers understood among themselves and verbally referred to by name, but did not record within the \DIFdelbegin \DIFdel{state diagram }\DIFdelend \DIFaddbegin \DIFadd{process flow }\DIFaddend because they were not sure how to represent it. When talking about features they requested a way to declare ``these are the typical paths'' or ``these two paths are similar''. When their discussions took place, they discussed specific workflows within the diagram, and discussed what happens in those \DIFdelbegin \DIFdel{work flows}\DIFdelend \DIFaddbegin \DIFadd{workflows}\DIFaddend . The researchers struggled with how to abstract away these workflows, and created additional diagrams, such as diagram depicted in Figure \ref{fig:researchgroup:b}.
...
\textbf{Table. } One of the researchers used tables to help him process sketches while onboarding. Said researcher created tables such as the one depicted in Figure \ref{fig:researchgroup:c}, and juxtaposed them against simplified sketches of \DIFdelbegin \DIFdel{state diagrams }\DIFdelend \DIFaddbegin \DIFadd{process flows }\DIFaddend in order in support of a detailed examination. The \DIFdelbegin \DIFdel{state diagram }\DIFdelend \DIFaddbegin \DIFadd{process flow }\DIFaddend in Figure \ref{fig:researchgroup:c} contained several paths that could be taken, and the researcher created tables to capture a particular workflow. They did so by listing each \DIFdelbegin \DIFdel{transition }\DIFdelend \DIFaddbegin \DIFadd{state in the row header, transitions }\DIFaddend along the column headers, and placing scraps \DIFdelbegin \DIFdel{depicting the data being passed within the states pertaining to the row headers}\DIFdelend \DIFaddbegin \DIFadd{in the intersecting cells}\DIFaddend . The researcher created several such tables to capture different \DIFdelbegin \DIFdel{work flows}\DIFdelend \DIFaddbegin \DIFadd{workflows}\DIFaddend .

A researcher reported that they created the table in Figure \ref{fig:researchgroup:c} to examine a set of corner cases. The table served as an aid to help them think through the \DIFdelbegin \DIFdel{state diagram}\DIFdelend \DIFaddbegin \DIFadd{process flow}\DIFaddend . The researcher reported that they used the table to simulate \DIFaddbegin \DIFadd{an }\DIFaddend algorithm, thinking through ``how this would be executed''. They reported that scraps were helpful in being able to move things. They reported that they normally would have created this diagram on a whiteboard, but in comparison, Calico provided ``lots of space'', made their table ``very clean'' because they did not have to redraw things. 

%``Dynamic execution of [his] diagram''. Would have enjoyed being able to play states.

The third researcher, in contrast to the others, \DIFdelbegin \DIFdel{often }\DIFdelend used Calico more frequently than the others to help him design while working alone. This researcher would prepare several canvases of sketches prior to a meeting, sketches including lists, tablets, \DIFdelbegin \DIFdel{state diagrams}\DIFdelend \DIFaddbegin \DIFadd{process flows}\DIFaddend , etc., to explain his designs. This researcher expressed that \DIFdelbegin \DIFdel{they normally would have presented }\DIFdelend \DIFaddbegin \DIFadd{he preferred to make }\DIFaddend his designs within Calico both because the other members were already using Calico, and \DIFdelbegin \DIFdel{he reported that }\DIFdelend \DIFaddbegin \DIFadd{because }\DIFaddend sharing sketches in Calico was faster because it was already a shared space.
...
\textbf{User interface mockups. } The researchers created prototypes of the user interface late in the design cycle, after \DIFdelbegin \DIFdel{creating several usable iterations. Rather, they turned to Calico to mockup user interfaces in order to support them during discussions}\DIFdelend \DIFaddbegin \DIFadd{having already created a working system that they could interact with. They used mockups during discussions to discuss changes to the user interface, and when designing new web pages}\DIFaddend . During these sessions, they imported screenshots of their current interface into Calico as a visual reference, and sketched interfaces without the use of scraps.

The \DIFdelbegin \DIFdel{final }\DIFdelend \DIFaddbegin \DIFadd{onboarded }\DIFaddend researcher created user interface mockups within Calico in order to help him in thinking through his design. In his task, he used a combination of screenshots and sketched mock-ups to first walk through how existing end-users write software code and submit into the system. From these sketches, he created a \DIFdelbegin \DIFdel{state diagram }\DIFdelend \DIFaddbegin \DIFadd{process flow }\DIFaddend to capture the interaction of the user and the flow of information.
...
\textbf{Spatially clustered text scraps. } In two meetings, the researchers generated a set of \DIFdelbegin \DIFdel{text-scraps }\DIFdelend \DIFaddbegin \DIFadd{text scraps }\DIFaddend as part of a brainstorm, and later grouped them into similar topics. They did not use formal arrows, but rather used grouping symbols, such as \DIFdelbegin \DIFdel{euler }\DIFdelend \DIFaddbegin \DIFadd{Euler }\DIFaddend diagrams, and drew arrows between the groups to indicate different types of relationships. 
...
\textbf{Source code. } \DIFaddbegin \DIFadd{In several of the meetings, one of the researchers pasted screenshots of source code into Calico to discuss his implementation. He pasted both psuedo code and actual javascript code. In meetings, he drew call outs from the screenshots and explain each method call, as in Figure \ref{fig:researchgroup:e}. }\DIFaddend

\subsection{Collaborative work}

Researchers conducted meetings using the two electronic whiteboards as well as their own laptops. During co-located meetings, they sometimes loaded both boards with Calico, but \DIFdelbegin \DIFdel{loaded on different canvases }\DIFdelend \DIFaddbegin \DIFadd{visually viewed different canvases on each whiteboard}\DIFaddend , or used the second board to display Google Docs. They reported that only one person usually sketched, while another took notes on \DIFdelbegin \DIFdel{a word document}\DIFdelend \DIFaddbegin \DIFadd{in Google Docs}\DIFaddend , and any \DIFdelbegin \DIFdel{remember }\DIFdelend \DIFaddbegin \DIFadd{remaining }\DIFaddend participants simply talked. 
...
\textbf{Continuity between design sessions.} The researchers also reported that Calico made it easier to stop and resume design sessions. One member reported that the ``biggest benefit was crossing space and time'', in which Calico provided a consistent virtual space to conduct meetings prior to the five day visit, during, and long after. \DIFdelbegin \DIFdel{The researchers conducted their designs in a shared space, but were able to launch their design spaces upon beginning their meetings for continuity}\DIFdelend \DIFaddbegin \DIFadd{All sketches and arrangement of canvases remain in a consistent location that all group members can always access. They did not need to worry about cleaning up any materials in their space or sharing whiteboard space with other research groups conducting meetings in the same space}\DIFaddend . 
...
\textbf{Providing an archival reference.} After the diagrams became out of date, the researchers reported that they used Calico as an ``archival reference''. The \DIFdelbegin \DIFdel{researcher }\DIFdelend \DIFaddbegin \DIFadd{researchers }\DIFaddend originally thought that they would continuously interact with their sketches, but instead used their sketches as visual references. The team eventually referenced the sketches less and less as they had internalized the contents of the sketches and could reference freely \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{without }\DIFaddend the need to have \DIFdelbegin \DIFdel{it }\DIFdelend \DIFaddbegin \DIFadd{the sketches }\DIFaddend in front of them. However, they reported that they valued having the sketches in Calico rather \DIFdelbegin \DIFdel{simple images}\DIFdelend \DIFaddbegin \DIFadd{than static photographs of whiteboards}\DIFaddend . They reported that it gave the sketches ``a sense of permanence'' because they could return and make adjustments to the sketches at any time, which they did. After undergoing a major refactoring, the content of the sketches in Calico became outdated, however, they did reference \DIFdelbegin \DIFdel{it again }\DIFdelend \DIFaddbegin \DIFadd{them still at times }\DIFaddend while moving forward. They reported that it was ``good \DIFaddbegin \DIFadd{to }\DIFaddend have a reference of what the architecture was like in the previous version''. They went back to past designs to see what they did before. When looking back at previous designs, they noted that ``it would have been nice to have annotations as to why this [previous design] wasn't going to work''. They remarked that while their sketched designs were most useful during implementation, they ``would turn back to the diagram because they forgot how they implemented something while writing''.
...
\DIFaddbegin \section{\DIFadd{Design behaviors}}
\label{chapter:evaluation:design-behaviors}
\DIFadd{In this section, I now turn to examining the design behaviors across all three sessions. I report on whether they occured or not, and whether the features were used. This is not an exhaustively review, i.e., I am not reporting on every instance or date each design behavior occurred. Rather, I am looking for proof of existence. This breaks down into three parts: 1) did I see proof of the designers performing the design behaviors, 2) did they use the features to perform them, 3) if so, which features.
}

%DIF > Start this section with a table. Summary of findings. Talk about each row of the table, because each row is a design behavior, in a little more detail
%DIF > Each sub section: design behavior... explanation. Now, rather than defending each one. 

\subsection{\DIFadd{Design Behaviors Summary}}

\DIFadd{Based on evidence from usage logs and the interviews, all design behaviors were observed as occurring at least once. More specifically, all 14 design behaviors occurred in the OSS group, 9 of the design behaviors occurred in interaction design group, and 13 of the design behaviors occurred in the research group. With respect to the usage of features, 13 of the 14 design behaviors for the OSS group were performed using Calico's features, 5 of the 9 design behaviors for the interaction design group were performed using Calico's features, and 12 of the 13 design behaviors were performed using Calico's features for the research group. Overall, the OSS group and the research group performed the design behaviors and were mostly supported by Calico features, while the interaction designers received only some benefit. 
}

\DIFadd{Table \ref{table:calico-version-two:designbehaviors} summarizes the features that supported each design behavior, which included scraps }\& \DIFadd{connectors, palette, intentional interfaces, and fading highlighter. Referring back to the table of features in Chapter \ref{chapter:calico-version-two}, Table \ref{table:calico-version-two:designbehaviors}, each feature was designed to support a specific set of design behaviors. In Table \ref{chapter:evaluation:designbehaviors-table}, all non-targeted features are shaded in gray.
}

\DIFadd{A summary of the design behaviors as they occurred is presented as follows:
}

\begin{enumerate}
	\item \DIFadd{\textbf{They drew different kinds of diagrams.} All groups performed this behavior using scraps, as well as with regular whiteboarding.
	}\item \DIFadd{\textbf{They produce sketches that draw what they need and no more.} All groups created scraps that were either sparse in details or notationally rich, depending on their needs. They also did so with regular sketching when not using scraps. The OSS group and the interaction design group both also used the palette to create low-detailed diagrams. The OSS group extensively used the fading highlight on low-detailed sketches.
	}\item \DIFadd{\textbf{They refine and evolve their sketches over time.} The OSS group refined sketches into box-and-arrow diagrams using scraps. The interaction design group refined one-dimensional plots into both two-dimensional plots and tables using regular sketching.
	}\item \DIFadd{\textbf{They use impromtu notations.} Both the research group and the interaction design group created custom notations using scraps and connectors. The interaction design group created custom icons using the palette. The OSS group created their own visual language using plain sketching by creating connectors with custom colors, shapes, and textures.
	}\item \DIFadd{\textbf{They move from one perspective to another}. All three groups used intentional interfaces to move between canvases of different perspectives. The OSS group and the research group benefited from intentional interfaces because chaining canvases provided an ordering for the canvases, which helped members of each group remember the meaning of their sketches within canvases. All groups also moved between perspectives within a single canvas.
	}\item \DIFadd{\textbf{They move from one alternative to another.} The OSS group and the research group used intentional interfaces to copy canvases and label them as alternatives. All three groups discussed alternatives verbally without using Calico.
	}\item \DIFadd{\textbf{They move from one level of abstraction to another.} The OSS group and the research group used intentional interfaces to step between canvases of different levels of abstraction. The OSS group used scraps to represent the software components they ``stepped into'' and ``out of''.
	}\item \DIFadd{\textbf{They perform mental simulations.} The OSS group and the research group used the fading highlighter to walk through the flow of data in their designs. The interaction designers created elaborate sketches that walked through the story of a user using regular sketching without any advanced features.
	}\item \DIFadd{\textbf{They juxtapose sketches.} The OSS group and the research group used scraps to position sketches next to one another for reference. The research group used the palette to copy content across canvases so that they could reference it. All groups created sketches of different perspectives in the same area to juxtapose by simply sketching them next to one another.
	}\item \DIFadd{\textbf{They review their progress.} All groups reported that they used intentional interfaces to review the sketches they have created. The OSS group and the research group also created handwritten lists that summarized the designs that they recently created.
	}\item \DIFadd{\textbf{They retreat to previous ideas.} The OSS group and research groups both used intentional interfaces to return to past designs and design decisions. The research group also returned to designs that were several months old.
	}\item \DIFadd{\textbf{They switch between synchronous and asynchronous work.} The OSS group and the research group had members that used intentioanl interfaces to deviate from the group discussion to their own canvas to reference content or create alternatives, however, this occurred rarely in the research group. 
	}\item \DIFadd{\textbf{They explain sketches to each other.} The OSS group and research group used the fading highlighter to explain designs. The OSS group reported finding this very useful to discuss data that passed between software components. 
	}\item \DIFadd{\textbf{They bring their work together.} The OSS group created new canvases that combined work from previous canvases.
}\end{enumerate}
...
\begin{figure}%DIF > 
  ...
  \subfigure[Composite of 10 minutes of fading highlighter use] {
   ...
   \caption {\DIFaddFL{The OSS group sometimes spent as long as 30 minutes walking through the data flow of a diagram using the fading highlighter.}}
...
\DIFdelbegin \section{\DIFdel{Minimally invasive}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{In the previous chapter, I reported on the experiences of three different groups using Calico. In this chapter, I bring together these observations to collectively answer the research questions posed in Chapter \ref{chapter:research-question}. 
}\DIFaddend 

\DIFdelbegin \DIFdel{Did it prevent you from doing what you normally would have done?
...
\section{\DIFadd{The degree to which features are minimally invasive and maximally cohesive}}

\DIFadd{//lets bring back the table. Here's the features, and here's the design behavior. Table that just summarizes it, features supported this this and this. We reduced the number of features, each one supports.
...
\section{\DIFadd{Strengths and weaknesses of each feature}}
\DIFadd{* individual features strengths and weaknesses }\DIFaddend - \DIFdelbegin \DIFdel{going against the grain. .. people are accustomed to their existing tools.
- too slow to bring up
}\DIFdelend \DIFaddbegin \DIFadd{what's my sense of the individual features and how well are they working.
...
//intentional interfaces }\DIFaddend - \DIFdelbegin \DIFdel{turned to word documents to supplement activity
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Did it encourage you to do anything that you wouldn}\DIFdelend \DIFaddbegin \DIFadd{are an advance over the grid. people do like the grid, but intentional interfaces gives us something that we didn}\DIFaddend 't have \DIFdelbegin \DIFdel{done?
- return to sketches..
.
}\DIFdelend \DIFaddbegin \DIFadd{before, such as names for canvases, relationships, order (and in some cases chronoloy)
}


\section{\DIFadd{Corroborating features}}
\DIFaddend 

\subsection{Cognitive dimensions analysis}
\DIFaddbegin \DIFadd{//not going to go through full analysis for each dimension.
...
The \DIFaddbegin \DIFadd{interview questions provide a high-level perspective of the overall usability on Calico and its features. In order to examine its usability in more detail, I perform a \textit{cognitive dimensions analysis} }\cite{Green96usabilityanalysis} \DIFadd{of Calico. The cognitive dimensions }\DIFaddend framework focuses on the \DIFdelbegin \DIFdel{artifact-user relationship, in order }\DIFdelend \DIFaddbegin \DIFadd{relatioship between the arifact and the user. The analysis framework provides a set of vocabulary and lens }\DIFaddend to consider the implications and trade-offs of representational features in the context of user activities. 

\DIFaddbegin \DIFadd{It should also be noted that software design sessions at the whiteboard have previously been examined using the cognitive dimensions framework }\citep{Petre2013BookChapter}\DIFadd{. In this section, I build on this past work, and use the findings as a comparison point for each cognitive dimension between Calico and the regular whiteboard.
}

\DIFaddend \subsubsection{Abstraction}
Abstraction refers to Calico's ability to support \DIFdelbegin \DIFdel{represent }\DIFdelend \DIFaddbegin \DIFadd{representations at }\DIFaddend different levels of \DIFdelbegin \DIFdel{abstractions}\DIFdelend \DIFaddbegin \DIFadd{abstraction}\DIFaddend . Since plain sketching inherently allows maximum flexibility in what can be represented, the tool \DIFdelbegin \DIFdel{supports representing many }\DIFdelend \DIFaddbegin \DIFadd{enables users to generate their own representations at different }\DIFaddend levels of abstraction\DIFdelbegin \DIFdel{well. Additionally, scraps further }\DIFdelend \DIFaddbegin \DIFadd{, much like on the traditional whiteboard. Calico further builds on top of basic sketching to support  using levels of abstraction with scraps and intentional interfaces. Scraps }\DIFaddend support encapsulating objects, given that scraps become inherently grouped when stacked. \DIFaddbegin \DIFadd{Intentional interfaces enable the user to depict different levels of abstraction across multiple canvases, and supports the user in navigating between these levels of abstraction by linking canvases. In contrast, a person using a regular whiteboard may be constrained by space, or may have difficulty navigating between levels of abstraction if the depicted figures are scattered across multiple whiteboards, posters, or pages. 
}

\DIFaddend In practice, the OSS group made the greatest use of abstractions across their designs\DIFdelbegin \DIFdel{, where they }\DIFdelend \DIFaddbegin \DIFadd{. They }\DIFaddend represented the same sketches from several perspectives, \DIFdelbegin \DIFdel{and copied scraps across canvases, both adding a greater or fewer number of sketched items}\DIFdelend \DIFaddbegin \DIFadd{reusing content between canvases with slight variations. They did so by copying scraps between canvases, and making those scraps larger to write more content inside that scrap}\DIFaddend . The interaction design group seldom used levels of abstraction, but rather they used several perspectives across canvases. The research group also used multiple levels of abstraction \DIFaddbegin \DIFadd{for state diagrams}\DIFaddend , however, they reported struggling with precisely what level of abstraction to use. \DIFdelbegin \DIFdel{The lowest common denominator in level of abstraction that their conversations took place were at the level of state transitions, however they verbally spoke about the state diagram at }\DIFdelend \DIFaddbegin \DIFadd{When sketching, they felt more comfortable visually depicting their system using state diagrams. However, during conversations, they assigned sections of the state diagrams names, indicating the presence of }\DIFaddend a higher level of abstraction \DIFdelbegin \DIFdel{, speaking in terms of groups. They did not have an adequate way to refer to these groups}\DIFdelend \DIFaddbegin \DIFadd{than just the individual states depicted. They only used these names verbally, and never wrote the names on the state diagram to avoid adding complexity to the diagram.
}

\DIFadd{Overall, Calico provided more benefit to depicting levels of abstraction than the regular whiteboard for the OSS group. Scraps and linking canvases using intentional interfaces supported them in their exploration. It is interesting to note that a single canvas may branch out to multiple canvases, each defining a particular component in more detail. Calico could be improved by providing direct linking between a scrap, and the canvas in which that scrap is defined in more detail}\DIFaddend .
...
Closeness of mapping refers to how closely \DIFdelbegin \DIFdel{do }\DIFdelend representations map to the referred concepts. \DIFdelbegin \DIFdel{Given that designers can }\DIFdelend \DIFaddbegin \DIFadd{Calico is similar to the whiteboard in this regard in that designers are at liberty to }\DIFaddend sketch any representation\DIFdelbegin \DIFdel{, the designers have total freedom in creating representations that represent their topics.
}\DIFdelend \DIFaddbegin \DIFadd{. If the designers decide that there is a more suitable representation, they are able to create a new sketch using that representation. When using scraps, Calico has both advantages and disadvantages with respect to sketching. A positive consequence of using scraps is that representations can be created more quickly by copying them, and are more easily minipulated because scraps can be moved and their attached connectors will move as well. A negative consequence of using scraps is that the visual appearance of scraps and connectors cannot be changed, i.e., scraps will always remain blue with a thin outline and connectors may only have one arrowhead. Its visual representation may not map as closely to the concept it depicts as a regular sketch. The user can compensate by sketching around a scrap, such as writing numbers next to a connector to indicate cardinality, but if a user moves the scrap, the cardinality number will not move with the scrap.
}

\DIFadd{For instance, }\DIFaddend OSS group members used square scraps to represent objects and long rectangular scraps to represent \DIFdelbegin \DIFdel{connectors}\DIFdelend \DIFaddbegin \DIFadd{event handlers}\DIFaddend . Further, in practice, the ability to import images and screenshots was an important feature to maintain closeness of mapping. Individuals from both the OSS group and \DIFdelbegin \DIFdel{researchers groups important }\DIFdelend \DIFaddbegin \DIFadd{the researcher group imported }\DIFaddend screenshots of code both to initiate new work, and to review the mapping between a sketched design and that code. In the case of reviewing code, a research group member drew ``call-outs'' from pieces of code to explain the logic used. In the case of the interaction designers, \DIFaddbegin \DIFadd{they reported that they could have represented the people interviewed using written names, but found that }\DIFaddend importing images of \DIFdelbegin \DIFdel{people }\DIFdelend \DIFaddbegin \DIFadd{the faces of those they interviewed }\DIFaddend evoked their memories much better\DIFdelbegin \DIFdel{than simply writing names}\DIFdelend .
...
\DIFdelbegin \DIFdel{Calico did }\DIFdelend \DIFaddbegin \DIFadd{Consistency refers to the degree to which features of structures and syntax are used the same way throughout. Calico does }\DIFaddend not provide an automated method to provide consistency. As with sketching, maintaining consistency between elements \DIFdelbegin \DIFdel{is mostly }\DIFdelend \DIFaddbegin \DIFadd{and sketches is }\DIFaddend up to the users to \DIFdelbegin \DIFdel{socially enforce between sketches }%DIFDELCMD < [%%%
\DIFdel{cite Marian's book chapter}%DIFDELCMD < ]%%%
\DIFdel{. Calico did}\DIFdelend \DIFaddbegin \DIFadd{``socially'' enforce }\cite{Petre2013BookChapter}\DIFadd{. Calico does}\DIFaddend , however, provide a set of features to propagate \DIFdelbegin \DIFdel{consistent }\DIFdelend \DIFaddbegin \DIFadd{consistency }\DIFaddend forward. Features such as copying scraps, the palette, and copying entire canvases allowed users to repeat elements with little effort. \DIFaddbegin \DIFadd{Scraps themselves can be manipulated and reused in design, decreasing the need redraw sketches as one would have on the whiteboard, which reduces the number of elements to maintain.
}

\DIFaddend The OSS group used repeated elements by copying scraps, using the palette, and copying canvases when creating alternatives. \DIFaddbegin \DIFadd{They created different perspectives of the same components, creating both user interface mockups and box-and-arrow diagrams, and maintained consistency in the coloring of connectors in box-and-arrow diagrams. }\DIFaddend The interaction design group created template canvases, which they copied to explore new perspectives. The \DIFaddbegin \DIFadd{interaction design group created a set of icons in the palette to represent actions in a user story, and reused these icons in their stories. The }\DIFaddend research group used the canvas copy feature to juxtapose the same sketch against different sketches. \DIFdelbegin \DIFdel{However, if entities did change within the sketches, the sketches became outdated}\DIFdelend \DIFaddbegin \DIFadd{They reported that maintaining consistency was difficult in dense state diagrams. In such complex diagrams, they needed to express design decisions within components, which lead to several impromtu notations that were not consistent with other diagrams, such as colored connectors and colored underlines}\DIFaddend . The research group reported that they copied the contents of sketches into more formal diagramming tools and into word documents, \DIFdelbegin \DIFdel{and that }\DIFdelend \DIFaddbegin \DIFadd{which did not always use the same syntax and structures. After transferring content into these formal tools, they no longer maintained the sketched in Calico, as }\DIFaddend the effort to maintain \DIFdelbegin \DIFdel{sketches in Calico }\DIFdelend \DIFaddbegin \DIFadd{them }\DIFaddend outweighed their benefit.

\DIFaddbegin \DIFadd{While the reusability of scraps and canvases within intentional interfaces did reduce the burden of maintaining consistency in designs in some cases, when put into practice, these were only a marginal improvement in comparison to the regular whiteboard. Users across the three groups manually maintained consistent use for features features of sturctures and syntax in their designs. In some cases, such as with the research group, users did not desire consistency, and valued out of date canvases as archival sketches.
}

\DIFaddend %- consistency brought by calico comes from copying, none beyond other, unless enforced by personal discipline.
...
Diffuseness refers to how much the meaning of sketches \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend spread out across \DIFdelbegin \DIFdel{many }\DIFdelend \DIFaddbegin \DIFadd{multiple }\DIFaddend sketches. The potential for an unlimited number of canvases with intentional interfaces may encourage \DIFdelbegin \DIFdel{diffuseness of diagrams}\DIFdelend \DIFaddbegin \DIFadd{designers to break up their designs across mutliple canvases, as opposed to the whiteboard where a designer may be more likely to be conservative in using space}\DIFaddend . Further, copying canvases and transferring content using the palette further encourage diagrams to be diffuse. \DIFaddbegin \DIFadd{Intentional interfaces can help mitigate navigating designs diffused across different canvases by providing links betweenthose  canvases, helping the user to find immediately relevant content.  }\DIFaddend However, this only proved to be the case in some situations. 
\DIFaddbegin 

\DIFaddend The OSS group created very diffuse diagrams by creating several copies of box-and-arrow diagrams with subtle changes, or different levels of abstraction. \DIFaddbegin \DIFadd{These canvases were grouped into chains by tagging the canvases as ``continuation'' in intentional interfaces, which they used to explore the design of a component in a step-wise fashion. }\DIFaddend The OSS group reportedly created use case diagrams in order to ``wrap their minds around what's going on'', referring to the diagrams in Figure \ref{fig:ossgroup:session2}. One of the \DIFaddbegin \DIFadd{members of the }\DIFaddend researchers further reported that without linking canvases with tags, he would have a difficult time recalling the meaning of contents in his past sketches. In contrast, the interaction designers attempted to reduce diffuseness by keeping \DIFaddbegin \DIFadd{related }\DIFaddend content contained within \DIFdelbegin \DIFdel{canvases. The researchers themselves }\DIFdelend \DIFaddbegin \DIFadd{its originating canvas. The research group }\DIFaddend also maintained all details of a state diagram within one sketch, wanting to also maintain a global view of their entire design at all times.

\DIFaddbegin \DIFadd{Overall, the intentional interfaces feature was the most useful tool in combating designs from becoming diffused to the point that relevant information in a design would be lost. Clusters within intentional interfaces provided a first means for grouping canvases, which helped separate projects between members in the research group, but not as much in the OSS group since they primarily used one cluster for all design meeting. Grouping and linking canvases using tags in intentional interfaces benefited the OSS group in linking canvases within a particular design session, and also helped one member of the research group in linking canvases within his own sessiosn as well. These features, while they do not prevent diffuseness of diagram, instead help make it manageable.
}

\DIFaddend %How diffuse were the representations created in calico? (i.e., were multiple diagrams required to show one concept?)
%
...
Hard operations refer to information that needs to be referenced or is nested. \DIFdelbegin \DIFdel{Similar to regular whiteboards, representations in Calico}\DIFdelend \DIFaddbegin \DIFadd{In a simple or small design, all parts of the design may be visible and there may be little need to reference outside information. However, in larger projects in a real world project, the design may be broken up across across many whiteboards, posters and papers. Similarly in Calico, representations }\DIFaddend may be diffused across several canvases \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{requiring the user to reference sketches outside the immediate space. Users may need to use the intentional interface feature to move between canvases to reference other sketches, or use the palette to move content so that it can be referenced in the active canvas.
}

\DIFaddend In the case of the OSS group, the developers needed to reference information that was \DIFdelbegin \DIFdel{referenced }\DIFdelend \DIFaddbegin \DIFadd{spread }\DIFaddend across several canvases. \DIFaddbegin \DIFadd{Members from their team reported that they did not need to do so often all members were already familiar with the depicted system. When they did need information to be available more immediately, they copied content onto the target canvas. The developer working with source code did so by pasting screenshots of his code into his canvas. }\DIFaddend In the case of the interaction designers, they pasted pictures of people they interviewed, however they needed to reference their notes \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend their interviews. The researchers, however, had a greater need to perform hard mental operations because of the scale of their state diagrams. The researchers who originally developed the system eventually internalized the diagram to a degree that they no longer needed to reference it, however the researcher that needed to be onboarded created additional diagrams, such as tables, in order to help him mentally step through the state diagram. \DIFaddbegin \DIFadd{In his case, he mitigated the need to reference other canvases by copying the pieces of the state diagram that he needed into his own canvases.
}

\DIFadd{Overall, the ability to copy content within Calico somewhat reduced the need to reference other canvases. Scraps, intentional interfaces, and the palette were helpful for both importing screenshots and copying content to the immediate working area.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{Hidden dependencies refers to representations that are dependent on one another, but the dependecy is not visible because it is either not declared or not in a person's field-of-view. }\DIFaddend As observed in past studies \DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{Dekel 2007}%DIFDELCMD < ]%%%
\DIFdelend \DIFaddbegin \cite{dekel2007notation}\DIFaddend , sketches that cross several spaces \DIFdelbegin \DIFdel{have several }\DIFdelend \DIFaddbegin \DIFadd{result in having }\DIFaddend dependencies on other spaces that are not explicitly stated. Sketches that begin in one canvas in Calico may extend onto another canvas, or across mediums\DIFaddbegin \DIFadd{, such as their own computer or paper, }\DIFaddend if the users are using Calico as a \DIFdelbegin \DIFdel{complement to other mediums. Intentional interfaces support support hidden dependencies }\DIFdelend \DIFaddbegin \DIFadd{complementary tool. This could occur, for example, when canvases depicted different perspectives or abstractions. Finding these other sketches is made easier }\DIFaddend to some extent \DIFdelbegin \DIFdel{allowing designers, grouping canvases first into }\DIFdelend \DIFaddbegin \DIFadd{by the intentional interfaces feature by allowing designers to group canvases into radial }\DIFaddend clusters, and further connecting canvases using tags. 
\DIFaddbegin 

\DIFaddend In the OSS group, the participants spread their diagrams across multiple canvases \DIFdelbegin \DIFdel{, which included multiple }\DIFdelend \DIFaddbegin \DIFadd{with different perspectives. Across these different canvases, they depicted several types of }\DIFaddend perspectives, such as \DIFdelbegin \DIFdel{the same diagram with different details revealed}\DIFdelend \DIFaddbegin \DIFadd{copies of the same sketch with different annotations}\DIFaddend , the same \DIFdelbegin \DIFdel{concept represented }\DIFdelend \DIFaddbegin \DIFadd{entities }\DIFaddend using different notations, and different pieces of larger diagrams. \DIFdelbegin \DIFdel{For example, a diagram included arrows at the edge }\DIFdelend \DIFaddbegin \DIFadd{In a canvas depicting a piece of a larger whole, an arrow pointed at the end }\DIFaddend of the canvas, \DIFdelbegin \DIFdel{representing data incoming from a different part of their system. In this case, all developers had pre-existing knowledge of the system, but still created lists that included details of diagrams on a separate canvas, and needed to return to this canvas. When asked to describe their diagrams, they found the linking of canvases by tagging helpful to return to diagrams created within the same session.   
}\DIFdelend \DIFaddbegin \DIFadd{depicting the flow of data onto another component off-canvas. However, most canvases did not have explicit dependencies between them, but they referenced the same element, and referred to content in other canvases. The developers worked around this issue by linking canvases with related content using intentional interface tags. When walking through their sketches during interviews, they remembered the chronology of their sketches by stepping through canvas links in the cluster view. They sometimes made explicit references between canvases as well.   
}

\DIFaddend The researcher group \DIFdelbegin \DIFdel{further had hidden dependencies because of similar issues with spreading their diagrams }\DIFdelend \DIFaddbegin \DIFadd{had issues similar to the OSS group, where dependencies between mutliple views in their design were diffused }\DIFaddend across multiple canvases. \DIFdelbegin \DIFdel{One }\DIFdelend \DIFaddbegin \DIFadd{In their case, information was spread across outside word documents, whiteboard sketches made prior to using Calico, and diagrams created in other tools. The research group members relied on their memory and cross checking documents to find dependencies between these different sources. Within Calico itself, one }\DIFaddend of the researchers reported that chaining \DIFdelbegin \DIFdel{also helped him }\DIFdelend \DIFaddbegin \DIFadd{canvases played an important role in helping him to }\DIFaddend recover hidden dependencies \DIFdelbegin \DIFdel{. The research group further had issues with both information spread across documents, outside sketches
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Drawing across multiple canvases }\DIFdelend \DIFaddbegin \DIFadd{and make sense of content in his canvases. He reported that many canvases would be senseless without being grouped in the same cluster as other canvases, and having been linked within intentional interfaces.
}\DIFaddend 

\DIFdelbegin \DIFdel{- intentional interfaces helped the researchers with }\DIFdelend \DIFaddbegin \DIFadd{Overall, the grouping mechanisms provided by intentional interfaces did prove useful in finding hidden dependencies. It does not solve the issue of revealing all }\DIFaddend hidden dependencies, \DIFdelbegin \DIFdel{the OSS group too
...
\caption{\DIFadd{What CDs Analysis Highlights about Calico}}\\
\hline
\DIFadd{\textbf{Cognitive Dimension} }& \DIFadd{\textbf{Calico Assessment Based on Use by Groups}}\\
\hline
\endfirsthead
\multicolumn{2}{c}%DIF > 
{\tablename\ \thetable\ \DIFadd{-- \textit{Continued from previous page}}} \\
\hline
\DIFadd{\textbf{Cognitive Dimension} }& \DIFadd{\textbf{Calico Assessment Based on Use by Groups} }\\
\hline
\endhead
...
\hline
\endlastfoot
\DIFadd{Abstraction	}&\DIFadd{Several levels of abstraction were depicted in sketches within Calico. Linking canvases with tagging within intentional interfaces were used to group canvases of the same topic across multiple levels of abstraction. When moving to another level of abstraction, scraps were copied to another canvas, }\DIFaddend in \DIFdelbegin \DIFdel{comparison to a separate lasso mode. Switching modes was further cumbersome on the large electronic whiteboardgiven that the buttons to toggle modes were sometimes not within arm's reach.
In comparison , those in the }\DIFdelend \DIFaddbegin \DIFadd{which they were defined in greater detail.  }\\
\hline
\DIFadd{Closeness of mapping	}&\DIFadd{Users were able to create representations that depicted the topics they discussed in most cases. Scraps were used to represent software components by creating box-and-arrow diagrams, however they could not represent all parts of box-and-arrow diagrams such as arrowheads, cardinality, arrow labels, etc. }\\
\hline
\DIFadd{Consistency	}&\DIFadd{Consistency within Calico needed to be manually enforced. Scraps, the palette, and intentional interfaces made propagating consistency forward by copying existing elements. }\\
\hline
\DIFadd{Diffuseness	}&\DIFadd{Designers were often diffused across several canvases. Intentional interfaces was helpful in combating diffuseness by making designs easier to navigate and manage. Chaining canvases using tagging was particularly helpful. }\\
\hline
\DIFadd{Error-proneness	}&\DIFadd{Calico did not inherently provide a method to avoid errors, users needed to manage this manually. The research group found that using scraps made working with large state diagrams easier, which reduced errors in syntax. }\\
\hline
\DIFadd{Hard mental operations	}&\DIFadd{Content diffused across several canvases caused users to need to reference other canvases. Copying content between canvases using intentional interfaces, scraps, and the palette mitigated the need to reference as often. The backwards and forwards navigation buttons also made referencing faster. }\\
\hline
\DIFadd{Hidden dependencies	}&\DIFadd{Dependencies within the design were sometimes hidden across canvases, but this was mitigated by linking canvases using intentional interfaces. Other content was not explicitly written, but only explained verbally or gestured during design sessions. }\\
\hline
\DIFadd{Premature commitment }&\DIFadd{All groups considered work done in Calico to all be provisional, and returned back to content previously created to iterate. Scraps, the palette, and intentional interfaces made it easier to explore alternatives by making it easier to manipulate content and create copies to deviate from. }\\
\hline
\DIFadd{Progressive evaluation }&\DIFadd{Calico does not directly support evaluation of content, but it did indirectly support users in manually reviewing their own work. Intentional interfaces helped users to move between their canvases and get a bird's eye view of canvases. }\\
\hline
\DIFadd{Provisionality }&\DIFadd{Plain sketching within Calico allows users to express indecision and options within sketches. Intentional interfaces further allowed users to declare canvases as alternatives. }\\
\hline
\DIFadd{Role expressiveness }&\DIFadd{Content created in Calico typically contained few details because of many hidden dependencies which were only revealed in verbal discussions and through gestures, resulting in representations with low role expressiveness. However, intentional interfaces somewhat improved role expressiveness by providing plenty of space to elaborate on designs.  }\\
\hline
\DIFadd{Secondary notation }&\DIFadd{Most support for secondary notations was provided by plain sketching, but scraps were annotated using colors to overlay additional information. Connectors further contained secondary information by color coding them. }\\
\hline
\DIFadd{Viscosity }&\DIFadd{Content created using Calico was notably less viscous than the whiteboard. Scraps and connectors made creating complex state diagrams easier to manage and manipulate. Copying content using scraps and the ``copy canvas'' button further made it easier to experiment with alternative solutions by deviating from an origin sketch. The research reported that they were hesitant to modify old sketches. All groups reported availability of free space encouraged them to sketch more. }\\
\hline
\DIFadd{Visibility }&\DIFadd{Sketches were diffused across several canvases, but chaining canvases using tagging in intentional interfaces made content easier to find across these canvases. Content was further made easier to juxtapose by arranging it on the canvas using scraps, or copying content across canvases using the palette or intentional interfaces.
}\label{table:discussion:cognitivedimensions}
\end{longtable}
\end{center}

\subsection{\DIFadd{Interviews}}
\DIFadd{//I've already made my interviews, now it's just my opinions
...
\DIFadd{//roll context into discussion. 
}


\DIFadd{First, the OSS group and the research group reported that Calico and its features did not prevent them from carrying out their design as they normally would have, while the interaction design group reported that Calico somewhat got in the way of their normal design activity. The OSS group reported that they did not feel any loss of expressive control in using Calico in comparison to the whiteboard, and reported that they normally would have performed many of the same activities on the whiteboards available in the immediate area. The research group reported that they formerly carried out their designs on the whiteboard, and transitioned to using Calico for the same activities. The }\DIFaddend interaction design group \DIFdelbegin \DIFdel{found that manipulating sketch content by using scrap gestures to be ``jarring'', in which they found the sudden shift from sketching to manipulating to happen too quickly.
They }\DIFdelend \DIFaddbegin \DIFadd{reported that it closely approximated the whiteboard, but they found that the quality of sketch input forced them to write large and that the system lagged at times, which was distracting during their sessions.
}

%DIF > Did it prevent you from doing what you normally would have done?
%DIF > - quality of sketching on whiteboards was largest compromise. Tablets helped, but was not the same (1,2,3)

\DIFadd{Second, there were some issues that were commonly intrusive among the three groups. Nearly all groups reported that the large electronic whiteboards diminished the quality of their own handwriting, forcing them to either write slower, or write larger. The OSS group connected to their Calico server using either tablets or their own computers, which they reported was preferred in producing clearer handwriting due to precise input. All groups used text-scraps to produce legible text as well. All groups also reported that launching Calico on their own machines was sometimes a hurdle. The OSS group reported that it was less of an obstacle for them because of the availability of tablets preloaded with Calico. The research group reported trouble setting up Calico on the laptops of new members of their group. Lastly, the members of the interaction design group were accustomed to a different set of gestures and interaction design patterns from their own set of digital sketching tools, particularly OneNote, which led to confusion with Calico's features, such as how to use scraps and gestures.
}

%DIF > Features that detracted
%DIF > - going against the grain... people are accustomed to their existing tools.
%DIF > - too slow to bring up
%DIF > - turned to word documents to supplement activity

\DIFadd{Third, all groups reported some positive changes to their existing design habits. All groups reported that they had a sensation of having more free space, and }\DIFaddend reported that they \DIFdelbegin \DIFdel{would rather use lasso as a different mode. Further, many groups desired the ability to change a scrap's border color and fill color, as they wanted to use a scrap's grouping capability or represent text, but found the scrap's blue color to be visually distracting. With respect to the palette feature , the palette }\DIFdelend \DIFaddbegin \DIFadd{created more sketches than they normally would have as a result. The research group reported that they created more complex sketches as a result of scraps and connectors, which helped them address a deeper level of complexity in their design. Both the OSS group and the research group reported that, since they did not feel a need to delete unused sketches, they returned to old sketches more often.
}
...
\hline
&\DIFaddFL{Basic sketching }&\DIFaddFL{Scraps }&\DIFaddFL{Palette }&\DIFaddFL{Intentional interfaces }& \DIFaddFL{Fading highlighter	}\\[5ex]
\hline
\DIFaddFL{Basic sketching }&  &	\DIFaddFL{OSS, Res }&\DIFaddFL{OSS, IxD, Res }&\DIFaddFL{OSS, Res }& \DIFaddFL{OSS, Res	}\\[5ex]
\hline
\DIFaddFL{Scraps }& \DIFaddFL{OSS, Res }&  & \DIFaddFL{OSS, IxD, Res }&  & \DIFaddFL{OSS, Res	}\\[5ex]
\hline
\DIFaddFL{Palette }& \DIFaddFL{OSS, IxD, Res }& \DIFaddFL{OSS, IxD, Res }&  &  &	\\[5ex]
\hline
\DIFaddFL{Intentional interfaces}&  &  & \DIFaddFL{OSS, IxD, Res }&	 & \DIFaddFL{OSS, Res	}\\[5ex]
\hline
\DIFaddFL{Fading highlighter}& 	 &	 &	 &	 & \\[5ex]
\hline
\end{tabular}
\label{table:cohesiveness}
\end{table}

%DIF > In this section, I discuss the interaction of each feature with all other features in the context of how they were used by the designers.

\DIFadd{\textbf{Basic sketching and features.} The use of this feature was augmented by all other features, which includes scraps, the palette, intentional interfaces, and the fading highlighter. Both the OSS group and the Research group scraps useful for manipulating sketches, which they found useful to move, resize, copy, rotate, and group sketches. They also found that using gestures made manipulating sketches to be helpful. All three groups found the palette to be helpful in supporting regular sketching. The palette }\DIFaddend supported sketching by reducing work by making sketches with repeated items more simple. Further, individuals created sketches which served as templates within the palette, and sometimes copied these onto a canvas. \DIFdelbegin \DIFdel{With respect to the intentional interfaces feature, the }\DIFdelend \DIFaddbegin \DIFadd{The OSS group and the research group found intentional interfaces to be helpful in their designs. It provided a }\DIFaddend bird's eye perspective \DIFdelbegin \DIFdel{provided by this feature encouraged designers }\DIFdelend \DIFaddbegin \DIFadd{in order to review and step through the sketches they created, and created the sense of plenty of space, which encouraged all groups }\DIFaddend to create more sketches. \DIFdelbegin \DIFdel{The fading highlighterfeature, while not used often, supported sketching over freeform }\DIFdelend \DIFaddbegin \DIFadd{With respect to the fading highlighter, the OSS groups and research groups reported that they found it helpful to trace over }\DIFaddend sketches without modifying \DIFdelbegin \DIFdel{them. However, in practice, designers felt more comfortable gesturing over sketches than using the fading highlighter. The instance in which the fading highlighter was used was when the designers had the tablet in their lap, and shared a display with an audience}\DIFdelend \DIFaddbegin \DIFadd{those sketches. They used it to support explanations of sketches, and in the case of the researchers, distributed explanations. Both groups reported, however, they sometimes while ``in the heat of the moment'' they forgot this feature existed, preferring to simply gesture over a sketch with their hands}\DIFaddend .

\DIFdelbegin \DIFdel{Scraps. The basic sketching and features available allowed designers to adapt scraps to a number of notations}\DIFdelend \DIFaddbegin \DIFadd{In contrast to other groups, the interaction design group did not find scraps helpful during their designs. They found that manipulating sketch content by using scrap gestures to be ``jarring'', in which they found the sudden shift from sketching to manipulating to happen too quickly. They reported that they would rather use lasso as a different mode. 
}

\DIFadd{\textbf{Scraps.} The use of this feature was augmented by basic sketching, the palette, and the fading highlighter. The OSS group and research group sketched over existing scraps to add detail and annotations}\DIFaddend . By using freeform sketching on and around scraps, the designers were afforded an ``escape from formalism'', and had flexibility in what they could represent in scraps. For example, some annotated cardinality on scraps, some added call-outs and annotations. The \DIFaddbegin \DIFadd{OSS, interaction design, and research groups reported that the }\DIFaddend palette increased the value of scraps by making scraps available across several canvases, which was performed, albeit rarely, by all groups studied. \DIFdelbegin \DIFdel{Intentional }\DIFdelend \DIFaddbegin \DIFadd{The OSS and research groups found that intentional }\DIFaddend interfaces supported scraps indirectly by the copy-canvas feature. In the OSS group, participants copied content across canvases, enlargened a scrap, and sketched out more detail pertaining to the enlarged scrap. The fading \DIFdelbegin \DIFdel{highlight }\DIFdelend \DIFaddbegin \DIFadd{highlighter }\DIFaddend feature was used by the OSS and research groups in group meetings to explain walkthroughs in which data is passed between scraps linked together by formal or informal arrows. The fading highlighter provided the benefit of both not leaving marks on the board, but also did not trigger any gestures.

\DIFdelbegin \DIFdel{Palette. Basic sketching enabled the palette to be flexible in what representations it could contain, however all groups reported the design to save sketches into the palette without first containing it in a scrap. Scraps }\DIFdelend \DIFaddbegin \DIFadd{\textbf{Palette.} The use of the palette was improved by basic sketching, and scraps. All three groups found that basic sketching made the palette more useful because they could populate it with the set of shapes that was useful to their current task. All three groups also reported that scraps }\DIFaddend themselves complemented the palette not only because scraps because they provided a vehicle to reuse sketches, but the ability to resize and rotate scraps made scraps much more reusable. Users of Calico Version One reported not using the palette because it could not be resized or rotated. In contrast, in Calico Version Two, the OSS group, for example, placed scraps in the shape of block arrows in the palette, and rotated them as needed. 

\DIFdelbegin \DIFdel{Intentional Interfaces.
The intentional interface feature did not receive any noticeable benefit from the other features, however the evaluations hinted at ways that it they could have. Designers saw the cluster-view as another canvas in itself and desired a way to sketch over, draw connections between, and tag canvases. 
They saw problems in that the radial cluster would re-arrange itself every timea new canvas was added, but saw organizing canvases as part of the design activity. With regard to scraps, designers expressed the desire to choose a scrap or circle an area of a canvas, and declare another canvas as pertaining to that scrap or area. With respect to the palette, the designers wished to create template canvases, such as a canvas with a pre-made table, or axis with four quadrants. Such templates helped guide their thinking. 
}\DIFdelend \DIFaddbegin \DIFadd{In feedback, all three groups reqested the ability to save a sketch to the palette without saving the sketch as a scrap first.
}\DIFaddend 

\DIFdelbegin \DIFdel{Fading Highlighter. This feature builds on top of }\DIFdelend \DIFaddbegin \DIFadd{\textbf{Intentional Interfaces.} The intentional interfaces feature was made more useful by the presence of the palette, and the fading highlighter. All three groups used the palette to make content from other canvases available in the current canvas. The palette provided a convenient mechanism to move parts of sketches between canvases. With respect to the fading highlighter, the OSS and research groups found the fading highlighter to be helpful in walkthroughs in the intentional interface. Both groups used intentional interfaces to look over what they had done, and would step through their design while using the fading highlighter to trace over sketches and scraps.
}

%DIF > The intentional interface feature did not receive any noticeable benefit from the other features, however the evaluations hinted at ways that it they could have. Designers saw the cluster-view as another canvas in itself and desired a way to sketch over, draw connections between, and tag canvases. They saw problems in that the radial cluster would re-arrange itself every time a new canvas was added, but saw organizing canvases as part of the design activity. With regard to scraps, designers expressed the desire to choose a scrap or circle an area of a canvas, and declare another canvas as pertaining to that scrap or area. With respect to the palette, the designers wished to create template canvases, such as a canvas with a pre-made table, or axis with four quadrants. Such templates helped guide their thinking. 
%DIF > 
%DIF > - basic sketching made references to other canvases
%DIF > 
%DIF > - scraps made references to other canvases

\DIFadd{\textbf{Fading Highlighter.} This feature builds on top of other feature, but does not receive benefits from }\DIFaddend other \DIFdelbegin \DIFdel{feature, but does not receive benefits from other features}\DIFdelend \DIFaddbegin \DIFadd{features}\DIFaddend .
...
\DIFdelbegin \section{\DIFdel{The role of multiple devices}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{With respect to recording the design process of professionals, a professor may use the recorded process as an examplar of official design. In past design courses, professors have provided students with case studies of professionals conducting design, such as videos of them at work. Selected portions of these design histories could be given to the students by the professors as examples. Previous proposals of tools have proposed similar methods of reviewing design histories, such as the Design practice stream (DPS) tools by Nakakoji }\cite{Nakakoji6035659}\DIFadd{, which plays back a history, and CogSketch }\cite{Forbus1149}\DIFadd{, which is used to understand how glyphs are used across all several domains.
}\DIFaddend 

\DIFdelbegin \section{\DIFdel{Role of Calico within the design process}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{With respect to recording the design process of students, Calico would enable professors to observe the design activity of students while outside of class. While professors can step in and provide guidance to students who become stuck in class, much work happens outside of class where they cannot provide guidance. The professors often can only see the final artifact or document that students submit. By having histories available, professors can review the histories to see if students are fixating on problems, adequately exploring alternatives, and so on.
}\DIFaddend 

\DIFdelbegin \section{\DIFdel{Education}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{It should be noted that the tools to review the usage logs generated by Calico are rudimentary. They provide a usage summary, as well as a set of screenshots of all actions taken, which can become highly verbose and be time consuming to review. There is an opportunity to improve this process so that professors and students can more effectively review their logs.
}\DIFaddend 

\DIFdelbegin \section{\DIFdel{Limitation of study}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend %DIF > \section{Limitations and threats to validity}
%DIF > 
%DIF > In this section, I address potential issues which may lead to threats in validity.
%DIF > 
%DIF > - Was not able to observe studies in person.
%DIF > - People interviewed may not have remembered correctly, or incorrectly interpreted logs.
%DIF > - All uses of Calico may have been idiosyncratic, and specific to the culture observed.
%DIF > - Observed different phases of the lifecycle for each group.
\DIFaddbegin 
...
%%% End: ***
 \newpage 
 \newpage \chapter{Conclusions \DIFaddbegin \DIFadd{and Future Work}\DIFaddend }
