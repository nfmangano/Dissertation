...
In this first version, I only target a subset of the design behaviors with the first version of Calico to demonstrate that I can indeed support the design behaviors. In later chapters, I use the insights gained from the first version to iterate and expand the feature set of this chapter to support the full set of design behaviors.

The first version targeted four of the design behaviors that I believed could be supported with a simple feature set. Specifically, it targeted design behavior 1, in which they draw different kinds of diagrams, design behavior 2, in which they only produce sketches that draw what they need, design behavior 3, in which they refine and evolve their sketches over time, and design behavior 5, in which they navigate from one perspective to another.

My first design of Calico was driven by my initial insights into how software designers work at the whiteboard. In adding features, I avoided those which would inhibit the flexibility and fluidity of the whiteboard. Features such as different modes, a selection lasso, etc. were not included because the movement between the sketch itself and menu bars as they may be disruptive to the fluidity of the whiteboard experience. Further, rather than including support by providing sketching support for an existing software design tool (such as with Marama Sketch \cite{Grundy}), I took the approach of building a basic sketching tool first, and adding features incrementally to this foundation.

I created three different iterations of Calico before arriving at Calico Version One. All three versions underwent internal testing, where I used each version in my own design meetings. These versions included several approaches to manipulating sketches, representing content, and navigating between different canvases of content. These past iterations included features such as lasso tools for manipulating content and a film strip metaphor for navigation.

From the iterative process, I arrived at four features. The features are: (1) a grid to manage multiple canvases in the workspace, (2) scraps that enable advanced manipulation of sketched content, (3) a palette save and reuse scraps, and (4) a gesture-based interaction scheme to tie all features to stylus-based input.

The remainder of this Chapter is organized as follows. Section \ref{calico} presents Calico and how it addresses the stated goals. Section \ref{experimentaldesign} presents my methods for evaluating Calico. My results are presented in Section \ref{results}. Section \ref{discussion} includes a discussion of the results and Section 8 reviews the threats to validity within the study. Lastly, Section \ref{conclusions} summarizes the chapter and its contributions.

\section{Features}
\label{calico}
As already alluded to, the features in Calico Version One were created to address four of the behaviors described in Chapter \ref{chapter:motivation}. To do so, I introduce four features, specifically: the grid, scraps, and the palette. In this section, I present these novel features, describe them in detail through various examples, and carefully relate them back to the design behaviors of Chapter \ref{chapter:motivation}. As a guide, Table \ref{table:1} shows a mapping between each of the four behaviors of Chapter \ref{chapter:motivation} and the main features of Calico.

Before discuss the features, it should be noted that Calico is intended to be used on an electronic whiteboard (or Tablet PC, in the case of sole person use), and is optimized to take its input from a digital stylus, not a mouse. The designer stands in front of the electronic whiteboard and uses the digital stylus to draw, write, and control Calico (see Figure \ref{fig:2}). The nature of this interaction shaped the design of Calico, particularly the mechanisms with which it enables designers to manipulate a design.
...
\begin{table}
\centering
\caption{Calico features as they address the behaviors discussed in Chapter \ref{chapter:motivation}}
\begin{tabular}{ p{4cm}p{2cm}p{9cm} }
\toprule
Behavior & Feature & Effect \\
\midrule

\multirow{2}{4cm} {Design Behavior 1: Use different kinds of diagrams}  &	Scraps	& Emerging notations are captured by scraps and can then be easily copied for reuse \\ 
	& Palette	 & Scraps are stored in a central location and can be reused anywhere \\
\midrule
\multirow{1}{4cm} {Design Behavior 2: Draw only what they need} & Scraps &	User-drawn shape can be preserved in order to enable low-detail, informal models; Strokes are grouped implicitly, which makes them moveable, stackable, and relatable \\
\midrule
...
Scraps address three of the behaviors discussed in Chapter \ref{chapter:motivation}: drawing different kinds of sketches (design behavior 1), creating low-detail sketches that have only as much as they need (design behavior 2), and the tendency to move from generic representations to more refined ones (design behavior 3).
...
The palette is the third feature that I incorporated in response to the behaviors discussed in Chapter \ref{chapter:motivation}. It specifically addresses the issue of using different types of sketches (design behavior 1).
...
Given my goal of supporting the specific design behaviors identified in Chapter \ref{chapter:motivation}, this section focuses on occurrences of these behaviors in the various pairs, as well as how the advanced functionality of Calico was used in the presence of these behaviors.
...
Bringing the results described in Chapter \ref{chapter:motivation} together, it is clear that Calico does provide support for the behaviors that I identified in Chapter \ref{chapter:motivation} and that designers are able to use the features in support of these behaviors when they so desire. However, it is also clear that features were not always used when they could have supported a certain behavior and that not every pair utilized every feature.
