...
Calico Version Two was evaluated by deploying it in \DIFdelbegin \DIFdel{the field to two software companies over }\DIFdelend \DIFaddbegin \DIFadd{three field sites, including a commercial open source software company, an interaction design company, and a geographically distributed research group. All groups had access to a large electronic whiteboard running a Calico client, as well as pen-based tablets to connect to a shared Calico server. Members of each respective group conducted extensive design sessions using Calico to perform their own work within their respective organization. In order to evaluate users, I collected and reviewed usage logs from the Calico server, and also conducted semi-structured face-to-face interviews. From this data, I evaluated the use of Calico's features in supporting the representations they created, their strategies for navigating between sketches, and how they collaborated on their sketches. 
}

\DIFadd{Overall, the field evaluations gave insight into how Calico supports design in a real world setting. First, the commercial open source software company provided insight in how a software team may use Calico to support ongoing with work among developers while coding, which involved group design discussions, and also personal sessions over both software components and software code. The interaction design group provided insight into how interaction designers may use Calico to help them create personas and storyboards, for example, by organizing pictures of those they interviewed in one- and two-dimensional plots. Lastly, the research group provided insight into how a set of distributed researchers may use Calico in support of a long-term collaboration. Their use included the development of a software system for }\DIFaddend a period of several \DIFdelbegin \DIFdel{weeks. }%DIFDELCMD < [%%%
\DIFdel{More to come after field deployment!}%DIFDELCMD < ]
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{months, which included intensive sessions of using Calico to design process flow diagrams to model the behavior of their software. Overall, the four features collectively supported all fourteen of the design behaviors both in ways that I did and did not expect. 
}\DIFaddend 
...
\textbf{Chapter \ref{chapter:evaluation} - \nameref{chapter:evaluation}.} Here I present our evaluation of the final version of Calico ``in the wild'' at \DIFdelbegin \DIFdel{two software companies. }%DIFDELCMD < [%%%
\DIFdel{Note: more to come after field trial!}%DIFDELCMD < ]%%%
\DIFdelend \DIFaddbegin \DIFadd{a commercial open source company, an interaction design company, and a distributed research group. A comprehensive evaluation is performed at each field site, evaluating their use of features to support the representations they used, how they navigated them, and how they collaborated. All three groups are also evaluated for evidence of how Calico's features supported each of the fourteen design behaviors}\DIFaddend .
...
\textbf{Chapter \ref{chapter:conclusions} - \nameref{chapter:conclusions}.} This chapter summarizes the contributions of my dissertation, and also provides some concluding remarks. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{\textbf{Chapter \ref{chapter:future-work} - \nameref{chapter:future-work}.} }\DIFdelend This final chapter suggests possible future avenues of research building upon the work introduced by this dissertation, including\DIFaddbegin \DIFadd{: 1) }\DIFaddend the concept of compositional notations, which builds on scraps to provide more targeted support for software design notations\DIFaddbegin \DIFadd{, 2) informal analysis of scraps in order to provide intelligent feedback on designs in progress such as critiquing the performance of an activity diagram, 3) contextualized sketching, which may import artifacts from an existing code repository to help with design discussions in Calico, and 4) design histories, which may help with reflecting on design activities performed}\DIFaddend .
...
As such, the summary of the approach in the remainder of this dissertation is that I will describe Calico (two \DIFdelbegin \DIFdel{versions with a interleaving chapter on design notations which was, in part, inspired by the first version of Calico}\DIFdelend \DIFaddbegin \DIFadd{different versions}\DIFaddend ) as well as the result of a qualitative evaluation of Calico in use at several local companies.
...
\begin{figure*}[tbh]
  ...
  \caption{The physical setup at the commercial OSS company\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }
...
\begin{figure*}[tbh]
  ...
  \caption{A tutorial of the usage of Calico given to the interaction design group\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }
...
\begin{figure*}[tbh]
  ...
  \caption{The physical setup of the research group\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }
...
\subfigure[Different perspective of Figure \ref{fig:ossgroup:session2:a}, Software architecture of the ``Alert'' event listener] {
  ...
  \subfigure[Use case diagram encompassing Figures \ref{fig:ossgroup:session2:a} to \ref{fig:ossgroup:session2:g}] {
   ...
   \caption {Representations used by the OSS group in their second design session in which they worked on an easily-understood representation that details the software architecture for handling different types of messages.}
